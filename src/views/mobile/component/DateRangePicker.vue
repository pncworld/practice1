...
<div class="flex md:hidden justify-center ">
    ...
    <div class="md:w-72 w-full flex relatvie flex-col md:flex-row ">
      <input
      type="button"
      :value="selectedRange == 'dayUnit' ? formatDate(dateRange[0]) +'~'+ formatDate(dateRange[1])  : (selectedRange == 'weekUnit' ? formatDate(weekRange[0][0]) +'~' + formatDate(weekRange[1][1]) : monthRange[0].year+'.'+ (monthRange[0].month+1) +'~' + monthRange[1].year+'.'+ (monthRange[1].month +1)) "
      @click="showCalender"
    ...
    <div class="absolute mt-9 w-full">
     <VueDatePicker v-if="selectedRange == 'dayUnit' && thisCalender" v-model="dateRange"
       class="w-full"
      :format="format"
      :locale="locale"
      :range="true"
      ...
      <VueDatePicker v-if="selectedRange == 'weekUnit' && thisCalender" v-model="weekRange"
      :format="format"
      :locale="locale"
      :range="true"
      placeholder="날짜를 선택하세요" 
      ...
      auto-apply
      :clearable="false"
      @update:modelValue="handleDateRangeChange" week-picker ></VueDatePicker> 

      <VueDatePicker v-if="selectedRange == 'monthUnit' && thisCalender" v-model="monthRange"
      :format="format"
      :locale="locale"
      inline
      :range="true"
      ...
      :clearable="false"
      @update:modelValue="handleDateRangeChange"  month-picker></VueDatePicker> 
...
function updateDateRange() {
  if(clickUnit.value == false) {
    ...
    weekRange.value = [
        [new Date(weekUnitStart), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd), new Date(weekUnitEnd)],  // 첫 번째 주 범위 (복사본 사용)
    ...
    break;
          ...
          startDate = {month: monthUnitStart.getMonth() , year:monthUnitStart.getFullYear() };
          monthUnitEnd.setDate(today2.getDate() - today2.getDay()+6)
          endDate = {month: monthUnitEnd.getMonth(), year:monthUnitEnd.getFullYear() };
          monthRange.value = []
