<template>
  <div>
    ...
    <br /><br />
    <div class="flex flex-col space-y-3 bg-gray-200 rounded-lg p-4 h-auto md:h-20 md:flex-row md:justify-between relative ">
  <!-- 상단 영역 -->
  <div class="flex flex-col md:flex-row md:justify-between items-center w-full relative">
    <div class="flex flex-col md:flex-row items-center w-full md:w-auto space-y-2 md:space-y-0 md:space-x-4 relative">
      <div class="flex items-center space-x-2">
        <span class="text-sm font-medium">일자 선택</span>
      </div>
      <DateRangePicker @update:dateRange="handleDateRangeUpdate" @update:unit="updateunit" class=" w-full absolute z-50"/>
    </div>
  ...
  <div class="p-4 flex w-full" >
    <!-- 여기에 하단 내용을 추가하면 됩니다 -->
     <div class="flex-1">
      ...
      {{  maxsalesDate }} <span v-if="currUnit == 'weekUnit'">월</span> {{ maxday }}<span v-if="currUnit == 'weekUnit'">주차</span>
    </p>
    <p class="font-bold">
    ...
    <p class="text-gray-600 font-bold text-2xl">
      {{  minsalesDate }} <span v-if="currUnit == 'weekUnit'">월</span>{{ minday }}<span v-if="currUnit == 'weekUnit'">주차</span>
    </p>
    <p class="font-bold">
...
<div class="w-full bg-white border border-gray-300 rounded-lg shadow-md" >
  ...
  <div class="w-full bg-white border border-gray-300 rounded-lg shadow-md mt-5" >
    ...
    <div class="grid grid-cols-3 w-full text-xl ">
      <div class="bg-gray-200"><span v-if="currUnit == 'dayUnit'">일자</span><span v-if="currUnit == 'weekUnit'">주차</span></div>
      <div class="bg-gray-200">건 수</div>
      <div class="bg-gray-200">실매출액</div>
    </div>
    <div v-for="i in result3" class="flex text-xl  ml-5" v-if="currUnit == 'dayUnit'">
    ...
    <div v-for="i in result2" class="flex text-xl  ml-5" v-if="currUnit == 'weekUnit'">
        <div class="flex-shrink-0 w-1/3"> {{ i.month}} 월 {{  i.week }} 주차</div>
        <div class="flex-shrink-0 w-1/3"> {{i.lngRecCnt}}</div>
        <div class="flex-shrink-0 w-1/3"> {{parseInt(i.lngActAmt).toLocaleString()}}</div>
    </div>
  ...
  const readsales = async () => {
    ...
    maxday.value = maxday2
      ...
      result2.value = result2.value.reduce((acc,item ,index) => {
        ...
        if(!acc[groupIndex]){
          acc[groupIndex] = { lngActAmt :0 , lngCustAmt :0 ,lngCustCnt :0 , lngDiscount:0 , lngRecAmt :0 ,lngRecCnt : 0, lngSalAmt:0 , lngSupplyAmt :0 , lngVAT:0 ,
            month: weekNumber.month , week : weekNumber.weekNumber
          }
        }
      ...
      console.log(result2.value)
      maxsales.value = result2.value.reduce((max, item) => (item.lngActAmt > max.lngActAmt ? item : max), result2.value[0]);
      minsales.value = result2.value.reduce((min, item) => (item.lngActAmt < min.lngActAmt ? item : min), result2.value[0]);
      sumCase.value = result2.value.reduce((sum, item) => sum + item.lngRecCnt, 0);
      sumSales.value = result2.value.reduce((sum, item) => sum + item.lngActAmt, 0);
      avgSales.value = result2.value.reduce((sum, item) => sum + item.lngActAmt, 0) / result2.value.length;
