  ...
  <div class="h-full" @click="handleParentClick">
    <div class="flex justify-between items-center w-full overflow-y-auto">
      <div class="flex justify-start  w-full pl-12 pt-4">
        <div class="flex justify-start">
          <h1 class="font-bold text-sm md:text-2xl w-full">사용자 페이지 접속 기록</h1>
        </div>
      </div>
    ...
    <div class="grid grid-cols-2 grid-rows-1 justify-between  bg-gray-200 rounded-lg h-24 items-center z-10">
      <div class="grid grid-cols-1 grid-rows-2">
        <Datepicker2 @startDate="startDate" @endDate="endDate" :mainName="'접속일자'" :closePopUp="closePopUp" ref="datepicker"
          :selectedGroup="selectedGroup" @excelDate="excelDate"  />
        <div class="flex justify-start items-center text-base text-nowrap font-semibold ml-40">아이디 :
          <div>
    ...
    <div class="w-full h-[82%]">
      <Realgrid :progname="'SYS01_003RPT_VUE'" :progid="1" :rowData="rowData" :reload="reload" :setFooter="false" 
                :documentTitle="'SYS01_003RPT'" :documentSubTitle="documentSubTitle" :exporttoExcel="exportExcel" />
    </div>
    </div>
  </template>
...
const searchButton = async () => {
  ...
  try {
    ...
    const res = await getuserPageAccessLogReport(groupCd.value, '0', storeCd.value, startDateResult, endDateResult, '')
    console.log(res)
