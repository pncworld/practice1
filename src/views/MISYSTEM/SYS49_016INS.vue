<template>
    <img src="../../assets/resetPassword.jpg" alt="" class="opacity-30 w-[87%] h-[88%] absolute">
    <div
        class="text-base text-left w-[45%] grid grid-cols-1 grid-rows-10 items-start justify-end relative opacity-100 mt-10 ml-[30%]">
        <div class="text-base font-bold opacity-100">(주)피앤시월드에서는 고객님의 개인정보 보호와 안전한 서비스 이용을 위해 항상 노력하고 있습니다.</div>

        <div class="font-semibold bg-blue-400">1.비밀번호를 변경하여 주세요</div>
        <div class="text-base font-bold"> 고객님께서는 오랫동안 비밀번호를 변경하지 않고 계십니다.</div>
        <div class="text-base font-bold"> 고객님의 소중한 정보를 보호하기 위하여 비밀번호를 변경하여 주시기 바랍니다.</div>
        <div class="font-semibold bg-blue-400">2.비밀번호 변경시 유의해 주세요</div>
        <div class="text-base font-bold"> 비밀번호는 6자리 이상 20자리 이하로 입력하십시오</div>
        <div class="text-base font-bold"> 비밀번호는 반드시 알파벳과 숫자를 하나 이상 포함해야 합니다.</div>
        <div class="text-base font-bold"> [ABCDEF]처럼 연속된 문자는 사용할 수가 없습니다</div>
        <div class="text-base font-bold"> 공백은 입력할 수 없습니다</div>
        <div class="text-base font-bold"> 비밀번호는 같은 문자만 연속해서 입력할 수 없습니다</div>

        <div class="flex flex-col justify-start mt-10">
            <div class="grid grid-cols-2 grid-rows-3 w-[50%] items-center justify-center">
                ...
                <div
                    class="border border-gray-500 p-2 flex h-auto items-center text-sm font-semibold justify-center bg-gray-100">
                    신규비밀번호 확인</div>
                <input type="password" v-model="newPassWordVali"
                    class="border border-gray-500 p-2 flex h-auto items-center text-sm font-semibold justify-center ">

            </div>
            <div class="flex justify-start ml-36 mt-5"><button class="button save" @click="saveButton">저장</button></div>
        </div>
    </div>
</template>
<script setup>
        ...
        if (result.isConfirmed) {
            ...
            try {
              ...
              if(res.data.List[0].value == '1'){
                 const res2 = await savePassWord(userlngGroup.value , userSequence.value ,newPassWord.value )
                 console.log(res2)
