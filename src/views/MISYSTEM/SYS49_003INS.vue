<template>
    ...
    <div class="grid grid-rows-1 grid-cols-[1fr,1fr] h-[85%] w-[90%] ">
        <div>
            ...
            <Realgrid :progname="'SYS49_003INS_VUE'" :progid="1" :rowData="rowData" :documentTitle="'SYS49_003INS'"
                class="mt-2" :selectionStyle="'singleRow'" @clickedRowData="clickedRowData" :initSelect="true"
                :moveFocusbyIndex="moveFocusbyIndex"  
                 :exporttoExcel="exceloutput" :changeNow="changeNow"
        ...
        <div class="grid grid-rows-[1fr,10fr] grid-cols-1 h-[85%] w-[110%]">
            ...
            <div
                ...
                <div class="border border-gray-500 p-1 flex space-x-3"><input type="text" v-model="value1" disabled
                        ...
                        v-model="value2"></div>
                        <div class="border border-gray-500 p-2 flex h-auto items-center text-sm font-semibold justify-center bg-gray-100 ">사용자명</div>
                <div class="border border-gray-500 p-1 flex space-x-3"><input type="text" v-model="value4"
                        @input="setValue4" class="border border-gray-500 w-64" :disabled="true"></div>
                        <div class="border border-gray-500 p-2 flex h-auto items-center text-sm font-semibold justify-center bg-gray-100 ">사용자ID</div>
                <div class="border border-gray-500 p-1 flex space-x-3"><input type="text" v-model="value3"
                        @input="setValue3" :disabled="true" class="border border-gray-500"></div>
                ...
                <div class="border border-gray-500 p-2 flex h-auto items-center text-sm font-semibold justify-center bg-gray-100">보안분류</div>
                <div class="border border-gray-500 p-1 flex">
                    <input type="text" v-model="value8" :disabled="true" class="border border-gray-500 p-1 flex">
                </div>
...
<script setup>
import { checkId, deleteUserInformation, getLockTypeList, getSecureList, getStoreListforUserEnroll, getSupervisorList, getUserEnroll, getUserPassWordEnroll, saveUserEnroll, saveUserPassWordEnroll } from '@/api/system';
...
const searchButton = async () => {
    ...
    try {
        ...
        const res = await getUserPassWordEnroll(groupCd.value, storecd, condition1.value, condition2.value)
        console.log(res)
...
const saveButton = async () => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            const res = await saveUserPassWordEnroll( selectedUserSequence.value, value7.value )
