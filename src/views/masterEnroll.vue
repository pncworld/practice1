<template>
    <div>
      ...
      <div class="space-x-0 flex justify-start items-center w-full ml-14">
        <div class="items-center w-3/12 justify-start flex whitespace-nowrap">매장 : &nbsp;&nbsp;<select class="border border-gray-800 rounded-md p-2 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 w-48 h-8 text-xs" v-model="storeCd" @change="getPosNo">
          <option value="0">전체</option>
          <option v-for="item in stores" :key="item.id" :value="item.lngStoreCode">{{item.strName}}</option>
        </select>
        </div>
        <div class="items-center w-3/12 whitespace-nowrap">포스번호 : <select class="border  border-gray-800 rounded-md p-2 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 w-48 h-8 text-xs"  v-model="posNo" @change="getAreaCd" >
          <option value="0">전체</option>
          <option v-for="item in posNos" :key="item.id" :value="item.lngCode">{{item.strName}}</option>
        </select></div>
        <div class="items-center w-3/12 whitespace-nowrap">지역 : <select class="border  border-gray-800 rounded-md p-2 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 w-48 h-8 text-xs" v-model="lngAreaCd" >
          <option value="0">전체</option>
      ...
      <ag-grid-vue :rowData="rowData" :columnDefs="colDefs" style="width: auto; height:700px"
        class="themeClass ag-theme-quartz" suppressRowClickSelection='false' rowSelection="multiple" rowMultiSelectWithClick='true' :gridOptions="gridOptions" @row-selected="onCellClicked"
      ...
      <div class="flex-grow basis-3/5">
        ...
        class="themeClass ag-theme-quartz" suppressRowClickSelection='false' rowSelection="multiple" rowMultiSelectWithClick='true'  :gridOptions="gridOptions"
        @grid-ready="onGridReady2"> </ag-grid-vue>
     </div>
     </div>
    </div>
  </template>
