<template>
  ...
  <div class="grid grid-rows-1 grid-cols-2 h-full w-full justify-center mt-2">
    ...
    <div class="w-[90%] ml-10 h-full">
        ...
        <Realgrid :progname="'ATT01_002INS_VUE'" :progid="1" :rowData="rowData" @clickedRowData="clickedRowData"  @selcetedrowData="selcetedrowData" @updatedRowData="updatedRowData" :selectionStyle="'singleRow'"  :labelingColumns="'blnExpireClass'" :valuesData="valuesData" :labelsData="labelsData" :addRow4="addRow" :deleteRow2="deleteRow" :addrowDefault="addrowDefault" :addrowProp="addrowProp"  :changeNow="changeNow" :changeValue2="changeValue2" :changeColid="changeColid" :changeRow="changeRow" @selectedIndex="selectedIndex"></Realgrid></div>
...
<script setup>
import { getChargerInfo } from '@/api/miattend';
...
if(afterSearch.value == false) {
  ...
  if(JSON.stringify(updateRow.value) === JSON.stringify(rowData.value) ) {
   ...
   const validateRow = updateRow.value.filter(item => item.strChargerName == '' || item.strName == undefined || item.lngChargerCode == '' || item.lngChargerCode == undefined ).length
  if(validateRow > 0 ) {
    Swal.fire({
      title: '경고',
  ...
  const validateRow2 = new Set(updateRow.value.map(item => item.lngChargerCode)).size !==  updateRow.value.length
