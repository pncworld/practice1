<template>
    <div class="h-full" @click="handleParentClick">
        <div class="flex justify-between items-center w-full overflow-y-auto">
            <div class="flex justify-start  w-full pl-12 pt-4">
                <div class="flex justify-start">
                    <h1 class="font-bold text-sm md:text-2xl w-full">월별 일자별 매출현황.</h1>
                </div>
            </div>
        ...
        <div class="grid grid-cols-3 grid-rows-2 bg-gray-200 rounded-lg h-24 items-center z-10 space-x-5 ">
            ...
            <div class="flex justify-start items-center h-8 space-x-1 ">
                ...
                <select name="" id="" class="w-20 h-8 rounded-lg flex justify-start" v-model="selectedCond2">
            ...
            <div class="flex justify-center items-center pr-10"><div class="text-base font-semibold w-24 text-nowrap">조회옵션 : </div><label for="retire"><input type="checkbox" id="retire" @click="exceptRetire">퇴직자제외</label></div>
        </div>
        <div class="w-full h-[82%]">
            <Realgrid :progname="'SLS02_006RPT_VUE'" :progid="1" :rowData="rowData" :reload="reload" :setFooter="true"
                :documentTitle="'SLS02_006RPT'" :setGroupFooter="true" :setGroupColumnId="'strAmountName'" :setMergeMode="false"
                :setFooterCustomColumnId="['lngEmpNo']" :setFooterCustomText="['합계']" :setGroupSumCustomColumnId2="['strCompName']"
...
import { getCondition4List, getSalesByBuyType, getSalesbyDateAndMonth } from '@/api/misales';
...
const searchButton = async () => {
    ...
    try {
        ...
        const res = await getSalesbyDateAndMonth(selectedGroup.value, selectedStores.value, selectedstartDate.value, selectedendDate.value ,selectedCond.value ,cardNo , empCode ,empName , custDeptName ,custCompName ,selectedCond3.value , selectedPos.value  )
        console.log(res)
