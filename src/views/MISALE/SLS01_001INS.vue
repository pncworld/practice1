<template>
    ...
    <div class="grid grid-rows-1 grid-cols-[7fr,7fr] justify-between  bg-gray-200 rounded-lg h-48 items-center z-10">
        <div class="grid grid-cols-1 grid-rows-2 mr-60">
            ...
            <div class="grid grid-rows-3 grid-cols-[1fr,4fr] justify-center items-center ml-10 -mt-8">
                ...
                <div class="flex">
                    <select name="" id="" class="w-32 h-8 rounded-lg ml-2" v-model="selectWeek">
                        <option :value="0">전체</option>
                        <option :value="i.lngCode" v-for="i in WeekDays">{{ i.strName }}</option>
                    </select>
                    <input type="number" class="w-52 h-8 rounded-lg ml-2 pl-1 text-right" v-model="targetSales">
                    <button class="button primary h-7 flex items-center ml-2 mt-1" @click="setMonthSales">적용</button>
                </div>
                <div class="text-base font-semibold text-nowrap justify-center flex items-center ml-3 mt-1">월매출목표액 :
                </div>
                <div class="flex">
                    <input type="text" class="w-52 h-8 rounded-lg pl-1 mt-1 ml-2 disabled:bg-gray-50 text-right"
                        v-model="targetMonthSales" disabled>
                </div>
                ...
                <div class="flex">
                    <div class="flex w-[300%]">
                        ...
                        <input type="file" class="hidden" id="hiddenFile" @change="readExcel" @click="resetFileInput">
        ...
        <div class="absolute z-[100]  h-20"  :style="{ top: popupTop, right: popupRight }" v-show="openPopUp">
            <table class="bg-[#f0f8ff] border border-gray-300 rounded-md shadow-sm w-full max-w-md mx-auto">
                <thead>
                    <tr class="border-b border-gray-200 ">
                        <th colspan="2" class="text-right pr-2">
                            <button class="button primary h-6  inline-block" @click="closePopup">닫기</button>
                        </th>
                    </tr>
                </thead>
                ...
                <tbody>
                    <tr class="border-b border-gray-200">
                        ...
                        <td class="px-4 py-2">
                            <input type="text" :value="clickedDate" disabled
                    ...
                    <tr class="border-b border-gray-200">
                        ...
                        <td class="px-4 py-2">
                            <input type="text" :value="clickedProj" @input="changeProj($event)"
                                class="w-full px-2 py-1 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td class="px-4 py-2">
                            <input type="text" :value="clickedComment" @input="changeComment($event)"
        ...
        <div class="flex space-x-52 w-[900px]">
            <div class="ml-14 text-nowrap">※ 1.샘플파일 다운받아 작성(Download버튼) 2.파일선택 후 "업로드 파일저장"버튼 클릭</div>
            <div><button @click="downloadSample" class="button primary h-6">download
                </button></div>
        </div>
    </div>
    ...
    reader.onload = (e) => {
        ...
        for (let i = 0; i < workbook.SheetNames.length; i++) {
            ...
            excelData.value.push(jsonData.slice(1).map((row) => ({
                ...
                목표금액: row[2].toString().includes(',') ? Number(row[2].replace(/,/g, '')).toString() : Number(row[2]).toString() ,
                비고: row[3],
            })));
        }
...
const saveUploadFile = async () => {
    ...
    try {
        const dates = excelData.value[selectSheet.value].map(i => i.일자).join(',')
        const projs = excelData.value[selectSheet.value].map(i => Number(i.목표금액.replace(/,/g, ''))).join(',')
        const comments = excelData.value[selectSheet.value].map(i => i.비고).join('\u2063')
        const res = await saveExcelDataPlan(lngstoregroup.value, lngstorecode.value, dates, projs, comments)
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    const comments = rowData.value.map(i => i.strComment).join('\u2063')
...
const calendarOptions = ref({
    ...
    eventClick: function (event) {
        ...
        console.log(event)
        const info = { dateStr: event.event.startStr , jsEvent : {clientX : event.jsEvent.clientX , clientY : event.jsEvent.clientY}}
...
watch(rowData, (newEvents) => {
    ...
    targetMonthSales.value = rowData.value.map(i => i.title).reduce((acc, cur) => acc + Number(cur), 0)
}, { deep: true });
...
const searchButton = async () => {
    ...
    try {
        ...
        rowData2.value = JSON.parse(JSON.stringify(res.data.List.map((item) => ({
...
const excelButton = () => {
    ...
    const headerRow = [
        "", "",
        { v: `${currentYear.value}년 ${currentMonth.value}월`, s: { alignment: { horizontal: "center" }, wrapText: true } },
        "",
        "", "", ""
    ];
    ...
    // 파일 저장
    writeFile(workbook, `${currentYear.value}년 ${currentMonth.value}월 계획 달력.xlsx`);
