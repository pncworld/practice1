/*--############################################################################
# Filename : SLS08_001RPT.vue                                                  
# Description : 매출관리 > 매출 정정 현황 > 매출 취소 현황                      
# Date :2025-05-14                                                             
# Author : 권맑음                     
################################################################################*/
<template>
  ...
  <div class="h-full" @click="handleParentClick">
    ...
    <div class="w-full h-[80%]">
      <Realgrid
        ...
        :setFooterExpressions="['sum', 'sum', 'sum', 'sum', 'sum']"
        :setFooterColID="[
          'lngActAmt',
          'lngCredit',
          'lngCash',
...
<script setup>
import { getCauseList, getSalesCancelData } from "@/api/misales";
/**
 *  매출 일자 세팅 컴포넌트
 *  */
...
const searchButton = async () => {
  ...
  try {
    ...
    if (
      ...
      setGroupColumnId.value.includes("strStore")
    ) {
      setRowGroupSpan.value = "dtmDate,strStore";
    } else if (
      tempCellUnite.value == true &&
      !setGroupColumnId.value.includes("strStore")
    ) {
    ...
    const res = await getSalesCancelData(
      selectedGroup.value,
...
const cellUnite = (e) => {
  if (e.target.checked) {
    ...
    if (setGroupColumnId.value.includes("strStore")) {
      setRowGroupSpan.value = "dtmDate,strStore";
    } else if (!setGroupColumnId.value.includes("strStore")) {
      setRowGroupSpan.value = "dtmDate";
    }
  } else {
