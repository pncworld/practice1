<template>
    <div class="h-full" @click="handleParentClick">
        ...
        <div class="grid grid-cols-2 grid-rows-1 justify-between  bg-gray-200 rounded-lg h-24 items-center z-10">
            <div class="grid grid-cols-1 grid-rows-3 mt-14">
                ...
                <Datepicker2 @endDate="endDate" @startDate="startDate" :closePopUp="closePopUp" ref="datepicker"
                    @excelDate="excelDate" :initToday="0" :limitEndDate="limitEndDate"></Datepicker2>
                <div class="flex flex-col justify-start items-start text-nowrap ml-40 ">
                    <div class=" text-nowrap flex justify-start items-center space-x-10 -ml-3 mt-2">
                        <div class="text-base font-semibold">조건 :</div>
                        <div class="flex space-x-5">
                            <label for="day">
                                <input type="checkbox" id="day" :value="0" v-model="selectedCond">
                                일자별
                            </label>
                            <label for="store"><input type="checkbox" id="store" :value="1"
                                    v-model="selectedCond">매장별</label>
                            <label for="sum"><input type="checkbox" id="sum" :value="2"
                                    v-model="selectedCond3">합계</label>
                            <label for="work"><input type="radio" id="work" name="cond" value="01"
                                    v-model="selectedCond2" checked>영업일</label>
                            <label for="approve"><input type="radio" id="approve" name="cond" value="02"
            ...
            <div class="ml-10  flex flex-col">
                <PickStoreSingle2 @lngStoreCode="lngStoreCodes" @lngStoreGroup="lngStoreGroup" @excelStore="excelStore"
                    @changeInit="changeInit" @lngSupervisor="lngSupervisor" @lngStoreTeam="lngStoreTeam"
        ...
        <div class="w-full h-[80%]">
            ...
            <Realgrid :progname="'SLS12_011RPT_VUE'" :progid="1" :rowData="rowData" :reload="reload" :setFooter="true"
                :setMergeMode="false" :setGroupCustomLevel="2" :setGroupFooter="setGroupFooter"
                :setGroupSumCustomColumnId2="['strStoreName']" :hideColumnsId="hideColumnsId"
                :setGroupColumnId="setGroupColumnId" :documentTitle="'SLS12_011RPT'"
                :documentSubTitle="documentSubTitle" :exporttoExcel="exportExcel" :setGroupOrderByColumnId="'dtmDate'">
            </Realgrid>
        </div>
    </div>
</template>
...
<script setup>
import { getCashReceiptDetail, getReportybyCashReceipt, getSalesCloseMaxDate } from '@/api/misales';
...
const searchButton = async () => {
    ...
    try {
        ...
        const res = await getReportybyCashReceipt(selectedGroup.value, selectedStoreAttrs.value, selectedStoreTeam.value, selectedStoreSupervisor.value, selectedStores.value, selectedstartDate.value, selectedendDate.value, reportType, selectedCond2.value)
        console.log(res)
...
watch((selectedCond), () => {
    ...
    if (selectedCond.value.includes(1)) {
        hideColumnsId.value = hideColumnsId.value.filter(item => item != 'strStoreName')
    ...
    setGroupColumnId.value = [...tempGroupColumnIds].sort((a, b) => b - a).join(',')
