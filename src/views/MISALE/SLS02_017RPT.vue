    ...
    <div class="grid grid-rows-[1fr,1fr,2.5fr,1fr] grid-cols-1 h-[83%]">
        <div>
            ...
            <Realgrid :progname="'SLS02_017RPT_VUE'" :progid="1" :rowData="rowData" :reload="reload"
                :exporttoExcel="exportExcel" :documentSubTitle="documentSubTitle" :documentTitle="'SLS06_005RPT'" :getJson="getJson" @getJsonData="getJsonData">
        ...
        <div>
            ...
            <div class="flex justify-between h-[90%] mt-2 space-x-80">
                <Realgrid :progname="'SLS02_017RPT_VUE'" :progid="2" :rowData="rowData2" :reload="reload"
                    :exporttoExcel="exportExcel" :documentSubTitle="documentSubTitle" :documentTitle="'SLS06_005RPT'"  :getJson="getJson" @getJsonData="getJsonData2">
                </Realgrid>
                <Realgrid :progname="'SLS02_017RPT_VUE'" :progid="5" :rowData="rowData5" :reload="reload"
                    :exporttoExcel="exportExcel" :documentSubTitle="documentSubTitle" :documentTitle="'SLS06_005RPT'"  :getJson="getJson" @getJsonData="getJsonData5">
                </Realgrid>
            </div>
        ...
        <div>
            ...
            <div class="flex  justify-between h-[86%] space-x-80">
                <Realgrid :progname="'SLS02_017RPT_VUE'" :progid="3" :rowData="rowData3"  :getJson="getJson" @getJsonData="getJsonData3"></Realgrid>
                <Realgrid :progname="'SLS02_017RPT_VUE'" :progid="6" :rowData="rowData6"  :getJson="getJson" @getJsonData="getJsonData6"></Realgrid>
            </div>
        </div>
        <div>
            ...
            <div class="flex  justify-between h-[90%] space-x-80">
                <Realgrid :progname="'SLS02_017RPT_VUE'" :progid="4" :rowData="rowData4"  :getJson="getJson" @getJsonData="getJsonData4"></Realgrid>
                <Realgrid :progname="'SLS02_017RPT_VUE'" :progid="7" :rowData="rowData7"  :getJson="getJson" @getJsonData="getJsonData7"></Realgrid>
            </div>
        </div>
    </div>
...
const excelButton = () => {
    ...
    setTimeout(() => {
        ...
        ws['!merges'] = [ { s: { r: 0, c: 0 }, e: { r: 0, c: 9 } }, // A1~J1 병합
        { s: { r: 2, c: 7 }, e: { r: 4, c: 9 } }  ]; //A1~ j1 병합
        ...
        utils.sheet_add_aoa(ws, [['시간대','조수','조수누계','객수','객수누계','매출액','매출액누계','객단가','객단가누계','비율']], { origin: "A8" });
        ...
        utils.sheet_add_json(ws, exportExcel1.value, { origin: "A9", skipHeader: true });
        ...
        utils.sheet_add_aoa(ws, [['세부항목','건수','금액','비율']], { origin: origin});
        utils.sheet_add_json(ws, exportExcel2.value, { origin: origin2, skipHeader: true });
        ...
        utils.sheet_add_aoa(ws, [['정산항목','건수','금액','비율']], { origin: origin3});
        utils.sheet_add_json(ws, exportExcel3.value, { origin: origin4, skipHeader: true });
        ...
        utils.sheet_add_aoa(ws, [['취소항목','건수','금액']], { origin: origin5});
        utils.sheet_add_json(ws, exportExcel4.value, { origin: origin6, skipHeader: true });
        ...
        utils.sheet_add_aoa(ws, [['메뉴그룹명','건수','금액','비율']], { origin: origin7});
        ...
        utils.sheet_add_json(ws, exportExcel5.value, { origin: origin8, skipHeader: true });
        ...
        utils.sheet_add_aoa(ws, [['카드사명','건수','금액','비율']], { origin: origin9});
        ...
        utils.sheet_add_json(ws, exportExcel6.value, { origin: origin10, skipHeader: true });
        ...
        utils.sheet_add_aoa(ws, [['세션구분','포스','작업시간','담당자명']], { origin: origin11});

        utils.sheet_add_json(ws, exportExcel7.value, { origin: origin12, skipHeader: true });
        ...
        ws[cell].s = {
          ...
          alignment: {
            horizontal: exceptionCenter.includes(cell) ? 'center' : !isNumber ? 'left' : 'right', // 가로 중앙 정렬 const origin2 = "A" + (secondTableStartRow+1); // exportExcel2.value.length
            vertical: exceptionCenter.includes(cell) ? 'center' : !isNumber ? 'left' : 'right',// 세로 중앙 정렬
          },
          border: {
      ...
      const range6 = ["A8","B8","C8","D8","E8","F8","G8","H8","I8","J8","A" + secondTableStartRow,"B" + secondTableStartRow,"C" + secondTableStartRow,"D" + secondTableStartRow,"A"+ (thirdTableRows+2),"B"+ (thirdTableRows+2),"C"+ (thirdTableRows+2),"D"+ (thirdTableRows+2),"A"+ (fourthTableRows+3),"B"+ (fourthTableRows+3),"C"+ (fourthTableRows+3),"G"+ secondTableStartRow,"H"+ secondTableStartRow,"I"+ secondTableStartRow,"J"+ secondTableStartRow,"G"+ (fifthTableRows+1),"H"+ (fifthTableRows+1),"I"+ (fifthTableRows+1),"J"+ (fifthTableRows+1),"G"+ (lastTableRows+2) , "H"+ (lastTableRows+2),"I"+ (lastTableRows+2),"J"+ (lastTableRows+2)]
      range6.forEach((cell) => {
        ws[cell].s = {
        font: {
            name: '맑은 고딕', // 폰트 이름
...
function getColumnStyles(worksheet) {
    ...
    worksheet['!cols'].forEach(col => {
        if (col.wpx) { // 픽셀 단위 너비 정보가 있는 경우
            colStyles += `<col style="width: ${col.wpx}px;">`;
        } else if (col.wch) { // 문자 단위 너비 정보가 있는 경우 (대략적인 너비)
