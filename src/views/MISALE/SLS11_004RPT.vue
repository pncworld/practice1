    ...
    <div class="h-full" @click="handleParentClick">
        ...
        <div class="grid grid-cols-2 grid-rows-1 justify-between  bg-gray-200 rounded-lg h-24 items-center z-10">
            <div class="grid grid-cols-1 grid-rows-3 mt-12">
                ...
                <div class=" text-nowrap flex justify-start items-center -space-x-7 ml-40 -mt-1">
                    <div class="text-base font-semibold">할인코드 :</div>
                    <div class="flex mt-2 items-center justify-start">
                        <v-select v-model="selectedDiscountCode" :options="discountCodeList" placeholder="전체"
                            label="strName" class=" custom-select5 " clearable="true" @click="resetVselect2" />
                    </div>
        ...
        <div class="w-full h-[80%]">
            ...
            <Realgrid :progname="'SLS11_004RPT_VUE'" :progid="1" :rowData="rowData" :reload="reload" :setFooter="true"
                :setMergeMode="false" :setGroupSumCustomColumnId2="setGroupSumCustomColumnId2" :setGroupCustomLevel="2"
                :setGroupFooter="true" :setGroupColumnId="setGroupColumnId" :selectionStyle="'group'"
                :setFooterCustomColumnId="['strStoreName']" :setFooterCustomText="['총합계']"
                :documentTitle="'SLS11_004RPT'" :documentSubTitle="documentSubTitle" :exporttoExcel="exportExcel">
            </Realgrid>
        </div>
    </div>
</template>
...
const searchButton = async () => {
    ...
    try {
        ...
        const res = await getDiscountRecords(selectedGroup.value, selectedStoreAttrs.value, selectedStoreTeam.value, selectedStoreSuperVisor.value, selectedStores.value, selectedstartDate.value, selectedendDate.value, selectedlngCode)
        console.log(res)
