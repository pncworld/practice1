    ...
    <div class="flex justify-between items-center w-full overflow-y-auto">
      <div class="flex justify-start  w-full pl-12 pt-4">
        <div class="flex justify-start">
          <h1 class="font-bold text-sm md:text-2xl w-full">요일별 매출 현황.</h1>
        </div>
      </div>
    ...
    <div class="grid grid-cols-2 grid-rows-1 justify-between  bg-gray-200 rounded-lg h-24 items-center z-10">
      <div class="grid grid-cols-1 grid-rows-2">
        <label for="daily" class="font-thin inline-flex">
          ...
          :selectedGroup="selectedGroup" @excelDate="excelDate" class="ml-[-126px]"></Datepicker2>
        </label>
        <div class="flex justify-start items-center text-base text-nowrap font-semibold ml-40 "> 요일명 : 
          <div class="flex ml-7 space-x-3 mt-1">
            <v-select v-model="selectWeekDay" :options="weekDay" placeholder="전체" label="strDName" class="w-48 !h-8 bg-white " clearable="true"/>
          </div>
        </div>
      </div>
      <div class="h-[75%] ml-5">
...
import { useStore } from 'vuex';
  ...
  const searchButton = async () => {
    ...
    try {
      ...
      if (selectedWeekDay.value == undefined || selectedWeekDay.value.length == 0 || selectedWeekDay.value == 0) {
