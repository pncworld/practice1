        ...
        <div class="flex justify-between items-center w-full overflow-y-auto">
            <div class="flex justify-start  w-full pl-12 pt-4">
                <div class="flex justify-start">
                    <h1 class="font-bold text-sm md:text-2xl w-full">결제 형태별 매출현황II</h1>
                </div>
            </div>
        ...
        <div class="grid grid-cols-2 grid-rows-1 justify-between  bg-gray-200 rounded-lg h-32 items-center z-10">
            <div class="grid grid-cols-1 grid-rows-3 mt-2">
                <Datepicker2 @startDate="startDate" @endDate="endDate" :closePopUp="closePopUp" ref="datepicker"
                    @excelDate="excelDate" :mainName="'기간'" />
                <div class="flex justify-center items-center mr-44">
                    <div class="text-base font-semibold">구분 :</div>
                    <div class="flex justify-center space-x-5 ml-5">
                        <select name="" id="" class="h-8 rounded-lg w-32" @change="getSub" v-model="selectedMainCode">
                            <option :value="'01'">직가맹</option>
                            <option :value="'02'">지역</option>
                            <option :value="'03'">매장유형</option>
                        </select>

                        <select name="" id="" class="h-8 rounded-lg w-32" v-model="selectedCode">
                            <option :value="-1">전체</option>
                            <option :value="i.lngCode" v-for="i in sublist">{{ i.strName }}</option>
                        </select>
                    </div>
                </div>
                <div class="flex justify-start items-center text-base text-nowrap font-semibold ml-32 ">조회조건 :
                    <div>
                    ...
                    <div>
                        <label for="sum" class="font-thin">
                            <input type="checkbox" id="sum" class="ml-5 " @change="seeSum">건수
                        </label>
                    </div>
                </div>
            </div>
            <div class="h-[75%] ml-5">
                <PickStorePlural @lngStoreCodes="lngStoreCodes" @lngStoreGroup="lngStoreGroup"
                    @lngStoreAttrs="lngStoreAttrs" @excelStore="excelStore" @changeInit="changeInit"></pickStorePlural>
            </div>
        </div>
        <div class="w-full h-[82%]">
            <Realgrid :progname="'SLS02_023RPT_VUE'" :progid="progId" :setDynamicGrid="true" :rowData="rowData"
                :reload="reload" :setFooter="true" :setFooterCustomColumnId="['strJoinType']" :setFooterCustomText="['합계']"
                :hideColumnsId="hideColumnsId" :mergeMask="''" :setMergeMode="false"
                :setRowGroupSpan2="setRowGroupSpan" :documentTitle="'SLS02_023RPT'" :documentSubTitle="documentSubTitle"
...
<script setup>
import { getJoinType, getSalesByPaymentType2Report, getStoreArea, getSubLease, isDynamicGrid } from '@/api/misales';
...
const searchButton = async () => {
    ...
    try {
        ...
        console.log(groupCd.value, storeCd.value, selectedstartDate.value, selectedendDate.value, tempSeeDaily.value ? 1 : 0, isDynamicGrid2.value, selectedMainCode.value, selectedCode.value, setCnt.value)
        const res = await getSalesByPaymentType2Report(groupCd.value, storeCd.value, selectedstartDate.value, selectedendDate.value, tempSeeDaily.value ? 1 : 0, isDynamicGrid2.value, selectedMainCode.value, selectedCode.value, setCnt.value)
        console.log(res)
...
const excelButton = () => {
    ...
    if(selectedCode.value == -1){
        ...
        cond += sublist.value.find(item => item.lngCode == selectedCode.value).strName 
    }
