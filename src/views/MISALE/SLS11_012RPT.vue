<template>
    <div class="h-full" @click="handleParentClick">
      ...
      <div class="grid grid-cols-2 grid-rows-1 justify-between  bg-gray-200 rounded-lg h-24 items-center z-10">
        ...
        <div class=" text-nowrap flex justify-start items-center space-x-10 ml-40 mt-2">
              <div class="text-base font-semibold">조건 :</div>
              <div class="flex space-x-10">
                <label for="day"><input type="checkbox" id="day" @click="byDay">일자별</label>
                <label for="store"><input type="checkbox" id="store" @click="byStore">매장명</label>
                <label for="unite"><input type="checkbox" id="unite" @click="showUnite">셀병합</label>
      ...
      <div class="w-full h-[80%]">
        ...
        <Realgrid :progname="'SLS11_012RPT_VUE'" :progid="1" :rowData="rowData" :reload="reload" :setFooter="true"  :setMergeMode="false" :setGroupSumCustomColumnId2="setGroupSumCustomColumnId2"
        :setGroupFooter="setGroupFooter"  :setGroupColumnId="setGroupColumnId"  :selectionStyle="'group'"
        :hideColumnsId="hideColumnsId" :setRowGroupSpan="setRowGroupSpan" 
          :documentTitle="'SLS11_012RPT'" :documentSubTitle="documentSubTitle" :exporttoExcel="exportExcel">
        </Realgrid>
      </div>
    </div>
  </template>
  ...
  <script setup>
  import { getCustomerList, getSalesbyCustomer, getSalesbyOrders } from '@/api/misales';
...
onMounted(async () => {
  ...
  const searchButton = async () => {
    ...
    try {
      ...
      const res = await getSalesbyOrders(selectedGroup.value,selectedStoreAttrs.value ,selectedStoreTeam.value , selectedStoreSuperVisor.value , selectedStores.value, selectedstartDate.value, selectedendDate.value, reportType.value)
      console.log(res)
