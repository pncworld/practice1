<!-- /*--############################################################################
# Filename : SLS14_044RPT.vue                                                  
# Description : 매출관리 > 기타 매출 현황 > 페이코 집계 현황           
# Date :2025-12-05                                                            
# Author : 권맑음                     
################################################################################*/ -->
<template>
  ...
  <div class="h-full" @click="handleParentClick">
    ...
    <div
      ...
      <div class="flex ml-12 space-x-5 mt-1">
        ...
        <div>
          <select name="" id="" class="w-44 h-7" v-model="cond">
        ...
        <div class="text-base font-semibold !pl-10">집계구분</div>
    ...
    <div class="w-full h-[70vh]">
      <Realgrid
        :progname="'SLS14_044RPT_VUE'"
        :progid="1"
        :rowData="rowData"
        :documentTitle="'SLS14_044RPT'"
        :setFooter="true"
...
import { getPayCoAggList } from "@/api/misales";
/**
 *  매출 일자 세팅 컴포넌트
 *  */
...
const searchButton = async () => {
  ...
  try {
    ...
    if (cond3.value == "1") {
      hideColumnsId.value = ["storeName"];
    } else if (cond3.value == "2") {
      hideColumnsId.value = ["saleDate"];
    } else {
      hideColumnsId.value = [];
    }
    const res = await getPayCoAggList(
      selectedGroup.value,
      selectedStore.value,
