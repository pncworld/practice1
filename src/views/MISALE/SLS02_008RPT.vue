<!-- /*--############################################################################
# Filename : SLS02_008RPT.vue                                                  
# Description : 매출관리 > 매출집계현황 > 월별메뉴이용실적현황.
# Date :2025-12-10                                                             
# Author : 권지안                    
################################################################################*/ -->
<template>
  ...
  <div class="h-full" @click="handleParentClick">
    ...
    <div class="w-full h-[80%]">
      <Realgrid
        :progname="'SLS02_008RPT_VUE'"
        :progid="1"
        :rowData="rowData"
        :reload="reload"
        :setFooter="true"
        :setFooterExpressions="['sum', 'sum', 'sum', 'sum', 'sum', 'sum', 'sum', 'sum', 'sum', 'sum', 'sum', 'sum']"
        :setFooterColID="['lngM1', 'lngM2', 'lngM3', 'lngM4', 'lngM5', 'lngM6', 'lngM7', 'lngM8', 'lngM9', 'lngM10', 'lngM11', 'lngM12']""
        :documentTitle="'SLS02_008RPT'"
...
<script setup>
import { getMonthlyMenuUseStatus } from "@/api/misales";
/**
 *  매출 일자 세팅 컴포넌트
 *  */
...
const checkFloor = () => {
  const chk1 = document.getElementById("thirdFloor").checked;   // 3층
  const chk2 = document.getElementById("ninethFloor").checked;  // 9층
  ...
  if (chk1 && chk2) {
    ...
    intRegNo = 9; // 둘 다 false → 초기값 유지
  }

  // console.log("intRegNo:", intRegNo);
...
const searchButton = async () => {
  ...
  try {
    ...
    const res = await getMonthlyMenuUseStatus(
      selectedGroup.value,
