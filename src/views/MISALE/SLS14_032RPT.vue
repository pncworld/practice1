<template>
    ...
    <div class="flex justify-between  bg-gray-200 rounded-lg h-16 items-center z-10">
      ...
      <div class="mr-28">
        <PickStoreRenew @lngStoreCode="lngStoreCodes" @lngStoreGroup="lngStoreGroup" @lngStoreAttrs="lngStoreAttrs" @excelStore="excelStore" :placeholderName="'선택'">
        </PickStoreRenew>
      </div>
...
const searchButton = async () => {
    ...
    try {
      ...
      reload.value = !reload.value
        const startDate = `${selectedstartYear.value}${String(selectedstartMonth.value).padStart(2, '0')}`
        const endDate = `${selectedendYear.value}${String(selectedendMonth.value).padStart(2, '0')}`

        const res = await getVATInfo(selectedGroup.value, selectedStores.value, startDate, endDate)
        console.log(res)
