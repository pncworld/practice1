<!-- /*--############################################################################
# Filename : SLS04_031RPT.vue                                                  
# Description : 매출관리 > 메뉴별 매출 현황> 일자별 매장별 메뉴 매출    
# Date :2026-07-13                                                             
# Author : 권지안                     
################################################################################*/ -->
<template>
  ...
  <div class="h-full" @click="handleParentClick">
    ...
    <div
      class="grid grid-cols-2 grid-rows-1 justify-between bg-gray-200 rounded-lg h-48 items-start z-10">
      <div class="grid grid-cols-1 grid-rows-5 h-full mt-1">
        <Datepicker2
          class=""
          @endDate="endDate"
          @startDate="startDate"
        ...
        <div
          ...
          메뉴구분
          <div class="flex ml-5 space-x-1 mt-1">
            <v-select
        ...
        <div class="flex justify-start items-center text-base text-nowrap font-semibold ml-12">
          메뉴명
          <div class="flex ml-8">
            <input
              v-model="selectedMenuName"
              type="text"
              class="border rounded h-8 px-2 w-72 text-sm font-normal bg-white"
              placeholder="메뉴명 입력" />
          </div>
        </div>
        <div
    ...
    <div class="w-full h-[80%] mt-1">
      ...
      <div id="print-area" class="h-[95%] w-full">
        <Realgrid
          :progname="'SLS04_031RPT_VUE'"
          :progid="1"
          :rowData="rowData"
          :reload="reload"
          :exporttoExcel="exportExcel"
          :documentSubTitle="documentSubTitle"
          :documentTitle="'SLS04_031RPT'"
...
<script setup>
import { getDailyByStoreMenuSales, getTableSearchCondition } from "@/api/misales";
...
const setGroupColumnId = ref(["strStore", "dtmDate"]);
const setGroupColumnId2 = ref("strStore,dtmDate");
...
const searchButton = async () => {
  ...
  try {
    ...
    const res = await getDailyByStoreMenuSales(
      selectedGroup.value,
    ...
    console.log("[SLS04_031RPT] getDailyByStoreMenuSales 응답:", res);
...
const ConditionSet = new Set([0, 3]);
/**
 * 엑셀 내보내기 함수
 */
...
const checkedReportTypes = new Set([0, 1, 2]);
