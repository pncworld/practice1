<template>
    <div class="h-full" @click="handleParentClick">
      ...
      <div class="grid grid-cols-2 grid-rows-1 justify-between  bg-gray-200 rounded-lg h-32 items-center z-10">
        <div class="grid grid-cols-1 grid-rows-3 mt-5">
          ...
          <div class="flex flex-col justify-start items-start text-nowrap ml-40 ">
            <div class=" text-nowrap flex justify-start items-center space-x-10 -ml-3 mt-2">
              ...
              <div class="flex space-x-10">
                <label for="unite"><input type="radio" id="unite" name="cond" :value="0" v-model="selecetedCond" checked>승인</label>
                <label for="store"><input type="radio" id="store" name="cond" :value="1" v-model="selecetedCond" >취소</label>
          ...
          <div class="flex justify-start ml-36 space-x-5">
            <div class="text-base font-semibold">조회조건 : </div>
            <label for="cunite"><input type="checkbox" id="cunite" checked @click="cellUnite">셀병합</label>
            <label for="sum"><input type="checkbox" id="sum" checked @click="showSum">합계</label>

            <div class="flex pl-24 space-x-5">
            <div class="text-base font-semibold">발급구분 : </div>
            <label for="all"><input type="radio" id="all" name="issues" v-model="selecetedCond2" :value="0" >전체</label>
            <label for="issue"><input type="radio" id="issue" name="issues" v-model="selecetedCond2" :value="1"  >발급</label>
            <label for="nissue"><input type="radio" id="nissue" name="issues" v-model="selecetedCond2" :value="2" >미발급</label>
          </div>
          </div>
        ...
        <div class="ml-10  flex flex-col">
          ...
          <div class="text-red-500 mt-5 mr-14 flex justify-end">
            <div>여신전문금융업법에 의거 3개월 이내의 자료만 조회 가능합니다.</div>
          </div>
        </div>
        <div></div>
  ...
  <script setup>
  import { getCashReceiptDetail, getCauseList, getPastSalesChanges } from '@/api/misales';
...
onMounted(async () => {
  ...
  const searchButton = async () => {
    ...
    try {
      ...
      const res = await getCashReceiptDetail(selectedGroup.value, selectedStoreAttrs.value ,selectedStoreTeam.value , selectedStoreSupervisor.value ,  selectedStores.value, selectedstartDate.value, selectedendDate.value, selecetedCond.value, selecetedCond2.value)
      console.log(res)
