    ...
    <div class="h-full" @click="handleParentClick">
        <div class="flex justify-between items-center w-full overflow-y-auto">
            <div class="flex justify-start  w-full pl-12 pt-4">
                <div class="flex justify-start">
                    <h1 class="font-bold text-sm md:text-2xl w-full">좌석별 매출현황.</h1>
                </div>
            </div>
        ...
        <div class="grid grid-cols-2 grid-rows-1 justify-between  bg-gray-200 rounded-lg h-24 items-center z-10">
            <div class="grid grid-cols-1 grid-rows-2">
                ...
                <div class="flex justify-start items-center text-base text-nowrap font-semibold ml-40">조건 :
                    <div class="ml-4">
                        <label for="byday"><input type="checkbox" id="byday" @click="showDay">일자별</label>
                    </div>
                    <div class="ml-3"><label for="bystore"><input type="checkbox" id="bystore" @click="showStore">매장명</label></div>
                    <div class="ml-4">
                        <label for="bysum"><input type="checkbox" id="bysum" @click="showSum">합계</label>
                    </div>
...
<script setup>
import { getSalesbySeats, getSalesByTimeAndSeats } from '@/api/misales';
...
const searchButton = async () => {
    ...
    try {
        ...
        const res = await getSalesbySeats(selectedGroup.value, selectedStoreAttrs.value, selectedStores.value , selectedstartDate.value , selectedendDate.value , selectedDay.value , selectedStore.value )
...
const showDay = (e) => {
    if(e.target.checked){
        selectedDay.value = 1
       hideColumnsId.value =  hideColumnsId.value.filter(item => item !='strDate')
