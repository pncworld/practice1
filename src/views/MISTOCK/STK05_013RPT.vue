<!-- /*--############################################################################
# Filename : STK05_013RPT.vue                                                  
# Description : 자재관리 > 주문 관리 > 주문출고서발행.               
# Date :2025-09-29                                                           
# Author : 권맑음                     
################################################################################*/ -->
<template>
  <!-- 조회조건 -->
  <div class="h-full" @click="handleParentClick">
    <div class="flex justify-between items-center w-full overflow-y-hidden">
      <PageName></PageName>
      <div class="flex justify-center mr-9 space-x-2 pr-5">
        <button @click="searchButton" class="button search md:w-auto w-14">
          조회
        </button>

        <button @click="excelButton" class="button excel">엑셀</button>
        <button @click="printButton" class="button print md:w-auto w-14">
          인쇄
        </button>
      </div>
    </div>
    <div
      class="grid grid-cols-3 grid-rows-2 justify-between bg-gray-200 rounded-lg h-24 items-start z-10">
      <div class="flex space-x-5 items-center">
        <Datepicker2
          :mainName="'청구일자'"
          @endDate="endDate"
          :initToday="1"
          @startDate="startDate"></Datepicker2>
      </div>

      <div class="flex space-x-5 ml-12 items-center">
        <PickStore
          :hideGroup="false"
          :hideAttr="false"
          @update:storeCd="lngStoreCode"
          :defaultStore="true"
          :defaultStoreNm="'전체'"
          :setDynamicStoreClass="'!h-8 !p-0'"
          :mainName="'출고매장'"></PickStore>
      </div>

      <div class="flex space-x-5 ml-12 items-center">
        <PickStore
          :hideGroup="false"
          :hideAttr="false"
          @update:storeCd="lngStoreCode2"
          :defaultStore="true"
          :defaultStoreNm="'전체'"
          :setDynamicStoreClass="'!h-8 !p-0'"
          :mainName="'입고매장'"></PickStore>
      </div>

      <div class="flex space-x-5 mt-2 items-center col-span-3 ml-12">
        <div class="text-base font-semibold">배송기사</div>
        <div>
          <select
            name=""
            id=""
            class="border border-black w-48 h-7"
            v-model="cond3">
            <option value="0">전체</option>
            <option :value="i.lngShippingCode" v-for="i in optionList">
              {{ i.strShippingName }}
            </option>
          </select>
        </div>
      </div>
    </div>
    <!-- 조회조건 -->
    <!-- 그리드 영역 -->
    <div class="w-full h-[82%]">
      <Realgrid
        :progname="'STK05_013RPT_VUE'"
        :progid="1"
        :rowData="rowData"
        :reload="reload"
        :setStateBar="false"
        :documentTitle="'STK05_013RPT'"
        @updatedRowData="updatedRowData"
        @clickedButtonCol="clickedButtonCol"
        @dblclickedRowData="dblclickedRowData"
        :checkRenderEditable="true"
        :checkRowAuto="false"
        :checkRowAuto2="true"
        :checkRowAuto2Col="'chk'"
        :documentSubTitle="documentSubTitle"
        :rowStateeditable="false"
        :exporttoExcel="exportExcel">
      </Realgrid>
    </div>
  </div>

  <div
    class="fixed inset-0 flex items-center justify-center bg-black/50 z-50"
    v-if="open">
    <!-- 팝업 내용 -->
    <div class="bg-white p-6 rounded-2xl shadow-lg w-[50vw] h-[50vh]">
      <div class="flex justify-between">
        <h2 class="text-xl font-bold mb-4">출고내역상세</h2>
        <div class="flex space-x-3">
          <button
            class="px-4 py-2 bg-gray-400 text-white rounded-lg hover:bg-blue-600"
            @click="open = false">
            닫기
          </button>
        </div>
      </div>
      <div class="grid grid-rows-1 grid-cols-3 border border-black mt-1">
        <div class="flex">
          <div
            class="text-base font-semibold bg-gray-100 border-r border-black w-[40%] flex items-center justify-center">
            출고매장
          </div>
          <div class="border-r border-black flex justify-center items-center">
            <input
              type="text"
              class="border border-black"
              v-model="scond"
              disabled />
          </div>
        </div>
        <div class="flex">
          <div
            class="text-base font-semibold bg-gray-100 border-r border-l border-black w-[40%] flex items-center justify-center">
            입고매장
          </div>
          <div
            class="border-r border-black flex justify-center items-center w-full">
            <input
              type="text"
              v-model="scond2"
              disabled
              class="border border-black w-[80%] h-[80%]" />
          </div>
        </div>
        <div class="flex">
          <div
            class="text-base font-semibold bg-gray-100 border-r border-l border-black w-[40%] flex items-center justify-center">
            출고번호
          </div>
          <div class="border-r border-black flex justify-center items-center">
            <input
              type="text"
              class="border border-black"
              v-model="scond3"
              disabled />
          </div>
        </div>
      </div>
      <div class="h-[60%] w-full mt-2">
        <Realgrid
          :progname="'STK05_013RPT_VUE'"
          :progid="2"
          :rowData="rowData2"
          :documentTitle="'STK05_013RPT'"
          :rowStateeditable="false"
          :setStateBar="false"></Realgrid>
      </div>
    </div>
  </div>
  <!-- 그리드 영역 -->
</template>

<script setup>
import {
  getOrderOutList,
  getOutDetailList,
  getShippingInfo,
  saveStockConfirm,
  saveStockConfirmAll,
} from "@/api/mistock";
import PageName from "@/components/pageName.vue";
import Realgrid from "@/components/realgrid.vue";
import { insertPageLog } from "@/customFunc/customFunc";
import { onMounted, ref } from "vue";

import Datepicker2 from "@/components/Datepicker2.vue";
import PickStore from "@/components/pickStore.vue";
import Swal from "sweetalert2";
import { useStore } from "vuex";

const optionList = ref([]);
const reload = ref(false);
const rowData = ref([]);
const afterSearch = ref(false);

/**
 * 선택한 매출 시작일자
 */

const cond = ref("0");
const cond2 = ref("0");
const cond3 = ref("0");
const cond4 = ref("");
const cond5 = ref("0");
const tempSeeStore = ref(false);
const reportCheckData = ref("0");
const progId = ref("1");
const mdate = ref("");
const dateValue = (e) => {
  mdate.value = e.replaceAll("-", "");
};
const lngStoreCode = (e) => {
  cond2.value = e;
};
const lngStoreCode2 = (e) => {
  cond3.value = e;
};
const store = useStore();
const loginedstrLang = store.state.userData.lngLanguage;

const datepicker = ref(null);
const closePopUp = ref(false);
/**
 * 매출 일자 안 라디오박스 닫기 위한 외부 클릭 감지 함수
 */

const handleParentClick = (e) => {
  const datepickerEl = datepicker.value?.$el;
  if (datepickerEl && datepickerEl.contains(e.target)) {
    return;
  }
  closePopUp.value = !closePopUp.value;
};

/**
 *  조회 함수
 */

const cond6 = ref(true);
const checkCond = (e) => {
  cond6.value = e.target.checked;
};

/* 매장 컴포넌트 관련 함수 */
const selectedGroup = ref();
const selectedStore = ref(0);

/**
 * 	화면 Load시 실행 스크립트
 */

onMounted(async () => {
  const pageLog = await insertPageLog(store.state.activeTab2);

  const res = await getShippingInfo(store.state.userData.lngStoreGroup);

  optionList.value = res.data.List;

  if (res.data.List.length > 0) {
    cond.value = res.data.List[0].lngStoreCode;
  }

  //console.log(store.state.storeCd);
});

/**
 * 그리드 초기화
 */

const initGrid = () => {
  if (rowData.value.length > 0) {
    rowData.value = [];
  }
};

//엑셀 버튼 처리 함수
const exportExcel = ref(false);
/**
 * 엑셀 내보내기 함수
 */

const excelButton = () => {
  const filterd =
    store.state.storeCd.filter((item) => item.lngStoreCode == cond.value)[0]
      ?.strName || "전체";

  const filterd2 =
    store.state.storeCd.filter((item) => item.lngStoreCode == cond2.value)[0]
      ?.strName || "전체";
  documentSubTitle.value =
    "청구일자 :" +
    sdate.value +
    "~" +
    edate.value +
    "\n" +
    "출고매장 : " +
    filterd +
    "\n" +
    "입고매장 : " +
    filterd2;

  // 엑셀 기능 실행
  exportExcel.value = !exportExcel.value;
};

const documentSubTitle2 = ref("");

// 엑셀 추출
const documentSubTitle = ref("");
const selectedExcelDate = ref("");

const selectedExcelStore = ref("");

const excelStore = (e) => {
  selectedExcelStore.value = e;
  //comsole.log(e);
};

const updatedrowdata = ref([]);
const updatedRowData = (e) => {
  console.log(e);
  updatedrowdata.value = e;
};

const searchButton = async () => {
  try {
    store.state.loading = true;
    const res = await getOrderOutList(
      store.state.userData.lngStoreGroup,
      cond.value,
      cond2.value,
      sdate.value.replaceAll("-", ""),
      edate.value.replaceAll("-", ""),
      cond3.value
    );

    console.log(res);

    store.state.loading = false;
    rowData.value = res.data.List;
    updatedrowdata.value = res.data.List.map((item) => ({
      ...item,
      Selected: false,
    }));
    afterSearch.value = true;
  } catch (error) {}
};

const sdate = ref("");
const startDate = (e) => {
  sdate.value = e;
};
const edate = ref("");
const endDate = (e) => {
  edate.value = e;
};

const tempColID = ref("");
const clickedButtonCol = (e) => {
  console.log(e);
  tempColID.value = e;
};

const clickedrowdata = ref("");

const open = ref(false);
const disabled = ref(false);

const rowData2 = ref([]);
const scond4 = ref("");
const scond5 = ref("");

const editableColId = ref("");
const tempStoreCd = ref("");
const tempStoreCd2 = ref("");
const dblclickedRowData = async (e) => {
  console.log(e);
  //   console.log(tempColID.value);

  tempStoreCd.value = e[1];
  tempStoreCd2.value = e[2];
  //tempMoveDate.value = e[4];

  editableColId.value = "";
  if (tempColID.value == "strMoveNo") {
    if (e[11] == "01") {
      disabled.value = false;
    } else {
      disabled.value = true;
    }

    //disabled.value = false;

    scond.value = e[3];
    scond2.value = e[5];
    scond3.value = e[6];

    try {
      store.state.loading = true;
      const res = await getOutDetailList(
        store.state.userData.lngStoreGroup,
        e[2],
        e[4],
        e[6]
      );

      console.log(res);

      store.state.loading = false;
      rowData2.value = res.data.List;
    } catch (error) {}
    open.value = true;
  }
};

const scond = ref("");
const scond2 = ref("");
const scond3 = ref("");

const printButton = () => {
  const filtered = updatedrowdata.value.filter((item) => item.chk == true);
  if (filtered.length == 0) {
    Swal.fire({
      title: "경고",
      text: "선택한 출고번호가 존재하지 않습니다.",
      icon: "warning",

      confirmButtonText: "확인",
    });
    return;
  }
  const storecds = filtered.map((item) => item.lngStoreCode).join(",");
  const storecds2 = filtered.map((item) => item.lngDesStoreCode).join(",");
  const ordercds = filtered.map((item) => item.strMoveNo).join(",");
  window.open(
    `http://222.231.31.99/Report/CRPrint.aspx?pCount=10&Report=STK05_013RPT_RPT&@P_lngStoreGroup=${
      store.state.userData.lngStoreGroup
    }&@P_lngStoreCode=${cond.value}&@P_lngDesStoreCode=${
      cond2.value
    }&@P_dtmFromDate=${sdate.value.replaceAll(
      "-",
      ""
    )}&@P_dtmToDate=${edate.value.replaceAll(
      "-",
      ""
    )}&@P_flag=1&@P_strMoveOutNoList=${ordercds}&@P_strSrcStoreCodeList=${storecds}&@P_strDesStoreCodeList=${storecds2}&@P_lngShippingCode=${
      cond3.value
    }`,
    "_blank",
    "width=1600,height=1200"
  );
};
</script>
