...
<template>
  ...
  <div class="h-full" @click="handleParentClick">
    <div class="flex justify-end items-center w-full overflow-y-hidden">
      <div class="flex justify-end mr-9 space-x-2 pr-5">
...
const getCardNo = async (a, b, c) => {
  try {
    const res = await getCardInfo22(a, b, 1);
    //console.log(res);
...
const searchButton2 = async () => {
  ...
  try {
    ...
    const res = await getBelongCust2(lngStoreCode.value, cond6.value, 1);
    ////console.log(res);
    rowData2.value = res.data.List;
...
const deleteButton = async () => {
  ...
  try {
    store.state.loading = true;
    const res = await deleteSpecialPrices2(forDeleteDatas.value.join(","));
...
const saveButton = async (e) => {
  ...
  try {
    ...
    if (updatedRows.value.deleted.length > 0) {
      ...
      try {
        const res = await deleteCustomors32(compcodes, custids, 1);
    ...
    if (updatedRows.value.updated.length > 0) {
      try {
        ...
        const res = await saveCreditCustomer3(
          id,
          flag,
          saleCompCodes,
          saleCustIds,
