      ...
      <div class="mt-3 ml-10 grid grid-cols-[1fr,3fr] grid-rows-2 gap-0 w-full">
        ...
        <div class="grid grid-rows-1 grid-cols-3 justify-center items-center ">
          <div class="border border-gray-400 h-full">
            <select name="blnInactive" id="" class="flex-1 border rounded-lg w-full h-full pl-1" @change="searchColumn"
              v-model="searchC1">
          ...
          <div class="border border-gray-400 h-full rounded-tr-lg">
            <select name="payDistinct" id="" class="flex-1 border rounded-lg w-full h-full pl-1" @change="searchColumn"
              v-model="searchC2">
              <option value="-1">전체</option>
        ...
        <div class="px-1 py-1 border border-gray-300 rounded-br-lg "><input type="text"
            class="border w-full h-full px-1 border-gray-400 rounded-lg" @input="searchword" v-model="searchWord"></div>
      </div>
      <div class="ml-10 mt-1 w-full h-full">

        <Realgrid class="w-full h-full" :progname="'MST36_001INS_VUE'" :progid="1" :rowData="updateRow"
          ...
          :labelingColumns="labelingColumns" :defaultSearchAllValue="-1" :changeNow="changeNow"
          :changeValue2="changeValue2" :changeColid="changeColid" :changeRow="changeRow" @selectedIndex="selectedIndex"
          :initSelect="true" :addRow4="addRow4" :deleteRow2="deleteRow3" :addrowDefault="addrowDefault"
          :addrowProp="addrowProp" @updatedRowData="updatedRowData" :rowStateeditable="false" :addField="'new'">
        </Realgrid>
    ...
    <div class="grid grid-cols-1 grid-rows-[1fr,9fr] ml-20">
      <div class="flex space-x-1 mt-10">
        <button class="bg-gray-100 h-12 rounded-t-lg font-bold p-2 border" @click="selectMenu(1)"
          :class="{ 'text-blue-400 bg-blue-100': selectedMenu == 1 }">기본설정</button>
        <button class="bg-gray-100 h-12 rounded-t-lg font-bold p-2 border disabled:bg-gray-50 disabled:text-gray-200 "
          ...
          :disabled="selectedPayDistinct">할인대상메뉴</button>
        <button class="bg-gray-100 h-12 rounded-t-lg font-bold p-2 border disabled:bg-gray-50 disabled:text-gray-200"
          @click="selectMenu(3)" :class="{ 'text-blue-400 bg-blue-100': selectedMenu == 3 }"
          :disabled="selectedMultiple">복합결제허용</button>
      </div>
      <div>
        <hr class="w-[90%] mt-0">
        <div v-show="selectedMenu == 1" class="mt-3 h-[46%]">
          ...
          <div class="grid grid-cols-[1fr,3fr,1fr,3fr] grid-rows-[1fr,2fr,1fr] mt-5 border rounded-lg w-[90%] h-[55%]">
            ...
            <div class="flex justify-center items-center border"><select name="" id=""
                class="border h-full w-full rounded-lg pl-2 disabled:bg-gray-200" v-model="gridvalue1" disabled>
            ...
            <div class="flex justify-center items-center border"><select name="" id=""
                class="border h-full w-full rounded-lg pl-2 disabled:bg-gray-200" disabled>
            ...
            <div class="grid grid-cols-1 grid-rows-2 h-full border">
              <div class="flex items-center mt-1 text-blue-400 font-semibold">*국문<input type="text" name="strName" id=""
                  class="h-full w-[80%] border rounded-lg pl-2 ml-2 font-thin text-gray-700 disabled:bg-gray-200"
                  v-model="gridvalue3" @input="changeInfo" :disabled="afterClickrow"></div>
              <div class="flex items-center mt-1 ml-1"> 영문<input type="text" name="strNameE" id=""
                  class="h-full w-[80%] border rounded-lg pl-2 ml-3 disabled:bg-gray-200" v-model="gridvalue4"
                  @input="changeInfo" :disabled="afterClickrow"></div>
            </div>
            <div class=" justify-center items-center bg-gray-100 border flex flex-col ">
              <div class="border h-full w-full flex items-center justify-center text-blue-400 font-semibold">*결제코드</div>
              <div class="border h-full w-full flex items-center justify-center text-blue-400 font-semibold">*사용여부</div>
            </div>
            <div class="grid grid-cols-1 grid-rows-2 ">
              <div><input type="text" name="lngCode" id=""
                  class="h-full w-full border rounded-lg pl-2 disabled:bg-gray-200" v-model="gridvalue5"
                  @input="changeInfo" :disabled="!(isNew == true && afterClickrow == false)"></div>
              <div class="space-x-5 border flex justify-left pl-2 items-center disabled:bg-gray-200"
                :disabled="afterClickrow"><label for="using1"><input type="radio" name="blnInactive" id="using1"
                    v-model="gridvalue6" value="0" @change="changeInfo" :disabled="afterClickrow">예</label><label
                  for="using2"><input type="radio" name="blnInactive" id="using2" v-model="gridvalue6" value="1"
                    @change="changeInfo" :disabled="afterClickrow">아니오</label></div>
            </div>
            <div class="flex justify-center items-center bg-gray-100 border">할인그룹</div>
            <div class="flex justify-center items-center border"><select name="" id=""
                class="border h-full w-full rounded-lg pl-2 bg-gray-200" v-model="gridvalue2" disabled>
      ...
      <option value="">선택</option>
            ...
            <div class="flex justify-center items-center border"><input type="date" max="9999-12-31"
                v-model="gridvalue7" name="dtmFromDate" @input="changeInfo" :disabled="afterClickrow"
                class="disabled:bg-gray-200"> ~ <input type="date" max="9999-12-31" v-model="gridvalue8"
                name="dtmToDate" @input="changeInfo" :disabled="afterClickrow" class="disabled:bg-gray-200"></div>
          ...
          <div class="grid grid-cols-[1fr,3fr,1fr,3fr] grid-rows-8 h-full mt-3 w-[90%]">
            ...
            <div class="space-x-5 flex  items-center border justify-left pl-2"><label for="discount1"><input
                  type="radio" id="discount1" name="lngRate" v-model="gridvalue11" value="0" @input="changeInfo"
                  :disabled="afterClickrow" class="disabled:bg-gray-200">금액</label><label for="discount2"><input
                  type="radio" id="discount2" name="lngRate" v-model="gridvalue11" value="1" @input="changeInfo"
                  :disabled="afterClickrow" class="disabled:bg-gray-200">비율</label> </div>
            <div class="justify-center items-center bg-gray-100 border flex">할인금액(비율)</div>
            <div class="flex justify-center items-center"><input type="number" name="lngAmt" id=""
                class="h-full w-full border rounded-lg pl-2 disabled:bg-gray-200" v-model="gridvalue12"
                @input="changeInfo" :disabled="afterClickrow"></div>
            ...
            <div class="space-x-5 flex  items-center border justify-left pl-2"><label for="autopay1"><input type="radio"
                  id="autopay1" name="blnAuto" v-model="gridvalue13" value="1" @input="changeInfo"
                  :disabled="afterClickrow" class="disabled:bg-gray-200">예</label><label for="autopay2"><input
                  type="radio" id="autopay2" name="blnAuto" v-model="gridvalue13" value="0" @input="changeInfo"
                  :disabled="afterClickrow" class="disabled:bg-gray-200">아니오</label></div>
            <div class="justify-center items-center bg-gray-100 border flex">할인한도금액</div>
            <div class="flex justify-center items-center"><input type="number" name="lngDiscAmtLimit" id=""
                class="h-full w-full border rounded-lg pl-2 disabled:bg-gray-200" v-model="gridvalue14"
                @input="changeInfo" :disabled="afterClickrow"></div>
            ...
            <div class="space-x-5 flex justify-left pl-2 items-center border"><label for="openmoney1"><input
                  type="radio" id="openmoney1" name="blnDrawer" v-model="gridvalue15" value="0" @input="changeInfo"
                  :disabled="afterClickrow" class="disabled:bg-gray-200">예</label><label for="openmoney2"><input
                  type="radio" id="openmoney2" name="blnDrawer" v-model="gridvalue15" value="1" @input="changeInfo"
                  :disabled="afterClickrow" class="disabled:bg-gray-200">아니오</label></div>
            <div class="justify-center items-center bg-gray-100 border flex">계산우선순위</div>
            <div class="flex justify-center items-center"><input type="number" name="lngPrior" id=""
                class="h-full w-full border rounded-lg pl-2 disabled:bg-gray-200" v-model="gridvalue16"
                @input="changeInfo" :disabled="afterClickrow"></div>
            ...
            <div class="space-x-5 flex justify-left pl-2 items-center border"><label for="receipt1"><input type="radio"
                  id="receipt1" name="blnReceipt" v-model="gridvalue17" value="0" @input="changeInfo"
                  :disabled="afterClickrow" class="disabled:bg-gray-200">예</label><label for="receipt2"><input
                  type="radio" id="receipt2" name="blnReceipt" v-model="gridvalue17" value="1" @input="changeInfo"
                  :disabled="afterClickrow" class="disabled:bg-gray-200">아니오</label></div>
            <div class="justify-center items-center bg-gray-100 border flex">잔금반환비율</div>
            <div class="flex justify-center items-center"><input type="text" name="lngChangeRateLimit" id=""
                class="h-full w-full border rounded-lg pl-2 disabled:bg-gray-200" v-model="gridvalue18"
                @input="changeInfo" :disabled="afterClickrow"></div>
            ...
            <div class="space-x-5 flex justify-left pl-2 items-center border"><label for="discountfor1"><input
                  type="radio" id="discountfor1" name="lngMenu" v-model="gridvalue19" value="0" @input="changeInfo"
                  :disabled="afterClickrow" class="disabled:bg-gray-200">전체 선택</label><label for="discountfor2"><input
                  type="radio" id="discountfor2" name="lngMenu" v-model="gridvalue19" value="1" @input="changeInfo"
                  :disabled="afterClickrow" class="disabled:bg-gray-200">부분 선택</label></div>
            <div class="justify-center items-center bg-gray-100 border flex">품목할인설정</div>
            <div class="flex justify-center items-center"><select name="lngDiscType" id=""
                class="border h-full w-full rounded-lg pl-2 disabled:bg-gray-200" v-model="gridvalue20"
                @change="changeInfo" :disabled="afterClickrow">
                <option value="">선택</option>
            ...
            <div class="space-x-5 flex justify-left pl-2 items-center border"><label for="allow1"><input type="radio"
                  id="allow1" name="blnDuplicate" v-model="gridvalue21" value="1" @input="changeInfo"
                  :disabled="afterClickrow" class="disabled:bg-gray-200">허용</label><label for="allow2"><input
                  type="radio" id="allow2" name="blnDuplicate" v-model="gridvalue21" value="0" @input="changeInfo"
                  :disabled="afterClickrow" class="disabled:bg-gray-200">비허용</label></div>
            <div class="justify-center items-center bg-gray-100 border flex">크롤링결제코드</div>
            <div class="flex justify-center items-center"><select name="" id=""
                class="border h-full w-full rounded-lg pl-2 disabled:bg-gray-200" disabled>
                <option value="">선택</option>

              </select></div>
            <div class="justify-center items-center bg-gray-100 border flex">단수처리방법</div>
            <div class="flex justify-center items-center"><select name="lngRoundType" id=""
                class="border h-full w-full rounded-lg pl-2 disabled:bg-gray-200" v-model="gridvalue22"
                @change="changeInfo" :disabled="afterClickrow">
                <option value="">선택</option>
            ...
            <div><input type="number" name="lngRound" id=""
                class="h-full w-full border rounded-lg pl-2 disabled:bg-gray-200" v-model="gridvalue23"
                @input="changeInfo" :disabled="afterClickrow"></div>
            <div class="justify-center items-center bg-gray-100 border flex rounded-bl-lg">세금계산방법</div>
            <div class="flex justify-center items-center"><select name="lngTax" id=""
                class="border h-full w-full rounded-lg pl-2 disabled:bg-gray-200" v-model="gridvalue24"
                @change="changeInfo" :disabled="afterClickrow">
                <option value="">선택</option>
            ...
            <div class="flex justify-center items-center"><select name="strIcon" id=""
                class="border h-full w-full rounded-lg pl-2 disabled:bg-gray-200" v-model="gridvalue25"
                @change="changeInfo" :disabled="afterClickrow">
                <option value="">선택</option>
        ...
        <div v-show="selectedMenu == 2" class="h-[80%] w-[90%]">
          ...
          <div class="mt-3  grid grid-cols-[1fr,3fr] grid-rows-2 gap-0 w-full">
            ...
            <div class=" border border-gray-300 rounded-tr-lg flex p-1">
              <select name="" id="" class="flex-1 border rounded-lg w-full h-full " @change="setSubCd"
          ...
          <Realgrid class="w-full h-full mt-5" :progname="'MST36_001INS_VUE'" :progid="2" :rowData="clickrowData4"
            @clickedRowData="clickedRowData2" :initCheckColumn="initCheckColumn" :initCheckValue="initCheckValue"
            :initCheckAct="initCheckAct" @checkedRowData="checkedRowData" :initSelect="true"
            :maintaincheckColumn="'menuCd'" :rowStateeditable="false" :changeNow="changeNow2"
            :changeValue2="changeValue3" :changeColid="changeColid2" :changeRow="changeRow2"
            @updatedRowData="updatedRowData2" @selectedIndex="selectedIndex2"></Realgrid>
        ...
        <div v-show="selectedMenu == 3" class="h-[90%] w-[90%]">
          ...
          <Realgrid class="w-full h-full mt-5" :progname="'MST36_001INS_VUE'" :progid="3" :rowData="filteredrowData5"
            :setAllCheck2="setAllCheck2" :uncheckColumn="'lngCode'" :uncheckValue="uncheckValue"
            :uncheckAct="uncheckAct" :maintaincheckColumn="'lngCode'" @checkedRowData="checkedRowData2"
            :rowStateeditable="false" @updatedRowData="updatedRowData3"></Realgrid>
...
<script setup>
import { getMenuListIncludeCommon, getPayCodeEnrollInfo, savePayCode } from '@/api/master';
...
const saveButton = () => {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      store.state.loading = true;
      try {
        const lngStoreCodearr = rowData.value.filter(item => item.deleted !== true).map(item => item.lngStoreCode)
        const strNamearr = rowData.value.filter(item => item.deleted !== true).map(item => item.strName)
        const strNameEarr = rowData.value.filter(item => item.deleted !== true).map(item => item.strNameE)
        const lngCodearr = rowData.value.filter(item => item.deleted !== true).map(item => item.lngCode)
        const blnInactivearr = rowData.value.filter(item => item.deleted !== true).map(item => item.blnInactive)
        const dtmFromDatearr = rowData.value.filter(item => item.deleted !== true).map(item => item.dtmFromDate)
        const dtmToDatearr = rowData.value.filter(item => item.deleted !== true).map(item => item.dtmToDate)
        const lngRatearr = rowData.value.filter(item => item.deleted !== true).map(item => item.lngRate)
        const lngAmtarr = rowData.value.filter(item => item.deleted !== true).map(item => item.lngAmt != undefined ? item.lngAmt.substring(0, item.lngAmt.length - 1) : 0)
        const blnAutoarr = rowData.value.filter(item => item.deleted !== true).map(item => item.blnAuto)
        const lngDiscAmtLimitarr = rowData.value.filter(item => item.deleted !== true).map(item => item.lngDiscAmtLimit)
        const blnDrawerarr = rowData.value.filter(item => item.deleted !== true).map(item => item.blnDrawer)
        const lngPriorarr = rowData.value.filter(item => item.deleted !== true).map(item => item.lngPrior)
        const blnReceiptarr = rowData.value.filter(item => item.deleted !== true).map(item => item.blnReceipt)
        const lngChangeRateLimitarr = rowData.value.filter(item => item.deleted !== true).map(item => item.lngChangeRateLimit)
        const lngMenuarr = rowData.value.filter(item => item.deleted !== true).map(item => item.lngMenu)
        const lngDiscTypearr = rowData.value.filter(item => item.deleted !== true).map(item => item.lngDiscType)
        const blnDuplicatearr = rowData.value.filter(item => item.deleted !== true).map(item => item.blnDuplicate)
        const lngRoundTypearr = rowData.value.filter(item => item.deleted !== true).map(item => item.lngRoundType)
        const lngRoundarr = rowData.value.filter(item => item.deleted !== true).map(item => item.lngRound)
        const lngTaxarr = rowData.value.filter(item => item.deleted !== true).map(item => item.lngTax)
        const strIconarr = rowData.value.filter(item => item.deleted !== true).map(item => item.strIcon)
        const checkedMenus = rowData.value.filter(item => item.deleted !== true).map(item => item.checkedMenu)
