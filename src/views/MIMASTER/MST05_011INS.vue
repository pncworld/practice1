...
<div class="flex flex-col ml-10 w-full h-full">
  <div class="flex h-1/3" v-show="afterSearch" >
  <div  class="grid grid-cols-5 grid-rows-2 gap-1 ml-20 mt-5 w-3/4 h-full ">
<div v-for="item in ScreenKeys" class="screen-muuri-sort-empty flex justify-center items-center" >
  <p v-if="item.strScreenName != ''" @click="showMenuKey(item.intScreenNo)" class="w-32 h-14 flex items-center justify-center"><button >{{ item.strScreenName }} </button><button @click.stop="editScreenKey(item.intScreenNo)"><font-awesome-icon :icon="['far', 'pen-to-square']" /></button></p> 
  <p v-else-if="item.new == true" @click="addScreenKey(item.intScreenNo)" class="w-28 h-28 flex items-center justify-center"><button > 화면키 추가 </button></p> 
  <p v-else class="w-28 h-28 flex items-center justify-center"></p> 

</div>
</div>
<div class="flex flex-col  ml-3 w-5 h-full mt-5 "><button class="border border-gray-500 w-full h-full" @click="showPrev"><font-awesome-icon icon="arrow-up-long" class="w-full  h-full"/></button><button class="border border-gray-400 h-full" @click="showScreenKeysOrder"><font-awesome-icon icon="bars" /></button><button class="border border-gray-500 w-full h-full" @click="showNext"><font-awesome-icon icon="arrow-down-long" class="w-full h-full"/></button></div>

</div>
<div v-show="afterSearch"  class="flex flex-col mt-10 ml-10 w-[90%] h-full">
 ...
 <div class="flex justify-end  w-full h-full ">
  ...
  :move="onMove"
   @end="onEnd"
   animation="200"
  class="grid grid-cols-5 grid-rows-6 gap-3 mt-4 w-full h-full  pb-10 pr-10"
>
  <div
  ...
  >
    <p>{{ item ? item.strKeyName : '' }}</p>
  </div>
</VueDraggableNext>
 <div class="flex flex-col w-5 h-5/6 mt-7">
  <div class="border border-gray-500 h-full" @click="prevMenuKey">
  <font-awesome-icon icon="arrow-up-long" class="w-full h-full"/></div>
  <div class="border border-gray-500 h-full" @click="nextMenuKey"><font-awesome-icon icon="arrow-down-long" class="w-full h-full"/></div>
...
import { deletetablePosMenuKey, get_category_info, getCategoryInfo, getMenuKeyList, getMenuList, getMultiLingual, getScreenList, getTablePosMenuKey, savetablePosMenuKey, tablePosMenuKey, tablePosMenuKey_v2 } from '@/api/master';
...
try {
    ...
    const res3 = await getScreenList( groupCd.value,nowStoreCd.value, nowStoreAreaCd.value , posNo.value  , 2)
...
const showMenuKey =(value) => {
  ...
  const start = ((currmenuKeyPage.value-1)*10) 
...
const showMenuKeys = () => {
  ...
  gridView.onCellItemClicked = function (grid, index, clickData) {
    ...
    console.log(currentSelectedMenuCode.value)
  return true;
}
...
const confirmaddScreenKey = () => {
  ScreenKeyOrigin.value.push({strScreenName : currentscreenKeyNm.value , intScreenNo : clickedScreenNo.value  })
  addscreenKey.value = false
  addfor10ScreenKey()
  console.log(ScreenKeyOrigin.value)
  currentscreenKeyNm.value = ''
