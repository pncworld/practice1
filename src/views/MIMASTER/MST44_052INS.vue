<template>
  ...
  <div class="z-50">
    <DupliPopUp :isVisible="showPopup2" @close="showPopup2 = false" :storeCd="nowStoreCd" :storeNm="clickedStoreNm"
      :areaCd="nowStoreAreaCd" :posNo="posNo" :progname="'MST44_052INS_VUE'" :dupliapiname="'DUPLIALLPOSDATA'"
  ...
  <span class="h-5 mt-3 flex justify-between items-center w-[900px] ml-[700px]  z-40">
    <h1 class="font-bold text-xl z-40">키오스크 메뉴키 배치정보</h1><span class="flex space-x-3 ml-32 pl-56 items-center">순서변경
      &nbsp; &nbsp;<label class="z-40"><input type="radio" name="changingMode" @click.stop="changingMode(1)"
          checked>교체하기 </label><label class="z-40"><input type="radio" name="changingMode"
        ...
        <div class="flex w-full h-full ">
          <VueDraggableNext v-model="items" :move="onMove" @end="onEnd" animation="200"
            ...
            <div v-for="(item, index) in items"
              ...
              <span class="flex flex-col items-center justify-center"><span
                  v-if="item && (item.strUserFileName != '' && item.strUserFileName != undefined)"
                  class=" h-20 w-28 flex justify-center"><img
                    :src="`http://www.pncoffice.net/MenuImage/Image/${item.strUserFileName}`" alt=""
                    class="h-full w-full"></span><span v-else-if="item && (item.strUserFileName == '')"
                  class="h-20 w-28 flex justify-center"><img src="../../assets/noimage2.png" alt=""
...
const showMenuKeys = () => {
  ...
  const columns = [
    ...
    {
      name: 'strUserFileName', fieldName: 'strUserFileName', header: {
        text: 'strUserFileName',
        styleName: 'header-style-2'
      }, numberFormat: '#,##0', visible: false
    },
    {
