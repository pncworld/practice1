<template>
  ...
  <div class="z-50">
    <DupliPopUp :isVisible="showPopup2" @close="showPopup2 = false" :storeCd="nowStoreCd"
      ...
      :dupliapiname="'DUPLICUSTOMORKEY'" :progid="1" :naming="'POS번호'" :naming2="'객층키'">
    </DupliPopUp>
  </div>
  ...
  <span class="h-5 mt-3 flex justify-between items-center w-[900px] ml-[700px]  z-40">
    <h1 class="font-bold text-xl z-40">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;객체키 설정</h1><span
  ...
  <div class="flex h-5/6 w-full -mt-5">
    <div class="flex flex-col w-3/5 h-5/6">
      ...
      <div class="h-full z-30" v-show="currentMenu == false">
        ...
        <div class="ml-10 mt-0 w-full h-full z-30">
          ...
          <Realgrid class="w-full h-full" :progname="'MST44_002INS_VUE'" :progid="1" :rowData="rowData"
            :showGrid="showGrid" :showCheckBar="false" @selcetedrowData="selcetedrowData" :searchWord="searchword1"
            :searchColId="'lngCode,strName'" :addRow="addRows" @updatedRowData="updatedRowData" :deleteRow="deleteRows"
            @clickedRowData="clickedRowData" :editableColId="'strName'"></Realgrid>
    ...
    <div v-if="showOtherKeys == false" class="grid grid-rows-[1fr,4fr] grid-cols-1 mt-5 w-full h-full z-20">
      <div class="flex h-full w-full items-start justify-center" v-show="afterSearch">
        ...
        <VueDraggableNext v-model="KeyList" :move="onMove" @end="onEnd" animation="200"
          class="grid grid-cols-4 grid-rows-4 mt-16 ml-6 w-[80%] h-full gap-1">
          <div v-for="(item, index) in KeyList" class="custom-grid-style"
            ...
            <span class="flex flex-col items-center justify-center w-full h-full rounded-lg"><span
                ...
                class="flex justify-center items-center">{{ item ? item.strKeyName : '' }}</span></span>
          </div>
        </VueDraggableNext>
...
const searchButton = async () => {
  ...
  try {
    ...
    if (KeyList.value.length < 16) {
      for (var i = 0; i < 16; i++) {
        const tindex = KeyList.value.findIndex(item => item.intKeySeq == i + 1)
        if (tindex == -1) {
...
const saveButton = async () => {
  ...
  if (JSON.stringify(confirmitem1.value) == JSON.stringify(updatedrowData.value) && JSON.stringify(confirmitem2.value) == JSON.stringify(KeyList.value)) {
    Swal.fire({
      title: '경고',
      text: '변경된 사항이 없습니다.',
      icon: 'warning',
  ...
  const notyeslength = updatedrowData.value.filter(item => item.lngCode == undefined || item.lngCode == '').length
  const notyeslength2 = updatedrowData.value.filter(item => item.strName == undefined || item.strName == '').length
  if (notyeslength >= 1) {
    Swal.fire({
      title: '경고',
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      store.state.loading = true;
      //console.log(updatedrowData.value)
      const lngCodes = updatedrowData.value.filter(item => item.sort != '법인용' && item.deleted != true).map(item => item.lngCode)
      const strNames = updatedrowData.value.filter(item => item.sort != '법인용' && item.deleted != true).map(item => item.strName)
      const intKeySeqs = KeyList.value.filter(item => item.lngKeyscrNo != undefined).map(item => item.intKeySeq)
      const strKeyNames = KeyList.value.filter(item => item.lngKeyscrNo != undefined).map(item => item.strKeyName)
      const lngKeyscrNos = KeyList.value.filter(item => item.lngKeyscrNo != undefined).map(item => item.lngKeyscrNo)
...
const addKey = () => {
  ...
  KeyList.value[clickedRealIndex.value] = { intKeySeq: clickedRealIndex.value + 1, strKeyName: clickedstrName.value, lngKeyscrNo: clickedCode.value }
