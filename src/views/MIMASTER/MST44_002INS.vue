<template>
  ...
  <div class="mt-3 ml-10 grid grid-cols-[1fr,3fr] grid-rows-2 gap-0 w-full">
    ...
    <div class="ml-10 mt-0 w-full h-full z-30">
      ...
      <Realgrid class="w-full h-full" :progname="'MST44_002INS_VUE'" :progid="1" :rowData="rowData" :showGrid="showGrid" :showCheckBar="false"  @selcetedrowData="selcetedrowData" :searchWord="searchword1" :searchColId="'lngCode,strName'" :addRow="addRows" @updatedRowData="updatedRowData" :deleteRow="deleteRows"></Realgrid>
...
KeyList.value = swappedItems.map((item, index) => ({
  ...
  const saveButton = async() => {
    ...
    if(JSON.stringify(confirmitem1.value)== JSON.stringify(commonKeyList.value) &&  JSON.stringify(confirmitem2.value)== JSON.stringify(KeyList.value) ) {
      Swal.fire({
        title: '경고',
        text: '변경된 사항이 없습니다.',
        icon: 'warning',
    ...
    if(result.isConfirmed){
      store.state.loading = true;
      const lngCodes = updatedrowData.value.map(item => item.lngCode)
      const strNames = updatedrowData.value.map(item => item.strName)
      const intKeySeqs = KeyList.value.map(item => item.intKeySeq)
      const strKeyNames = KeyList.value.map(item => item.strKeyName)
      const lngKeyscrNos = KeyList.value.map(item => item.lngKeyscrNo)
 ...
 const currentpaymentCd = ref(3)
  ...
  const addKey =() => {
    ...
    KeyList.value[clickedRealIndex.value] = {strKeyName: clickedstrName.value, lngKeyscrNo : clickedCode.value  }
