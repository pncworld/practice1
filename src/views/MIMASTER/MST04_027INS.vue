<!-- /*--############################################################################
# Filename : MST04_027INS.vue                                                  
# Description : 마스터관리 > 자재 마스터2 > 배송 주기 관리Ⅱ(발주)                     
  # Date :2025-08-22                                                           
  # Author : 권맑음                     
################################################################################*/ -->
<template>
...
onMounted(async () => {
  ...
  if (groupcds.includes(store.state.userData.lngStoreGroup)) {
    ...
    const res = await getStockDetail(store.state.userData.lngStoreGroup, "03");
    ...
    await Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const blnsat = updatedrowdata.value
          ...
          .join("\u200b");
        const res = await saveStockCycle(
          store.state.userData.lngStoreGroup,
          storecds,
          0,
          leadtimes,
...
const excelButton = () => {
  const name =
    optionList.value.filter((item) => item.lngDetail == cond.value)[0]
      ?.strName || "전체";
  documentSubTitle.value = clickedStoreNm.value + "\n" + "자재특성 : " + name;
