<template>
  ...
  <div class="z-50">
    <DupliPopUp :isVisible="showPopup2" @close="showPopup2 = false" :storeCd="nowStoreCd" :storeNm="clickedStoreNm"
      :areaCd="nowStoreAreaCd" :posNo="posNo" :progname="'MST01_011INS_VUE'" :dupliapiname="'DUPLIALLPAYKEY'"
  ...
  <span class="h-5 mt-3 flex justify-between items-center w-[900px] ml-[700px]  z-40">
    <h1 class="font-bold text-xl z-40 ml-20 mt-3">결제키 배치</h1><span
      class="flex space-x-3 ml-32 pl-56 items-center mt-3">순서변경 &nbsp; &nbsp;<label class="z-40"><input type="radio"
          name="changingMode" @click.stop="changingMode(1)" checked>교체하기 </label><label class="z-40"><input type="radio"
          name="changingMode" @click.stop="changingMode(2)">밀어내기</label><button class="whitebutton z-40"
  ...
  <div class="flex h-5/6 w-full -mt-5">
    <div class="flex flex-col w-2/5 h-5/6">
      ...
      <div class="h-full" v-show="currentMenu == false">
        ...
        <div class="ml-10 mt-0 w-full h-full">
          ...
          <Realgrid class="w-full h-full" :progname="'MST44_055INS_VUE'" :progid="1" :rowData="rowData"
    ...
    <div class="w-5/12 h-[70%] mt-[122px] ml-[122px]">
      ...
      <VueDraggableNext v-if="changeGrid == false" v-model="KeyList" :move="onMove" @end="onEnd" animation="200"
        ...
        <div v-for="(item, index) in KeyList" class="custom-grid-style"
          ...
          <span class="flex flex-col items-center justify-center w-full h-full rounded-lg border border-gray-500 "><span
              class="flex justify-center items-center" v-if="item.gp == 1">결제그룹코드</span><span
              class="flex justify-center items-center" v-if="item.gp == 0">결제코드</span><span
      ...
      <VueDraggableNext v-if="changeGrid == true" v-model="KeyList2" :move="onMove" @end="onEnd" animation="200"
        class="grid grid-rows-5 grid-cols-5 h-full w-full gap-1 ">
        <div v-for="(item, index) in KeyList2" class="custom-grid-style"
          :class="{ '!bg-orange-500 !border-blue-700': clickedMenuKey == index && clickedScreenOrMenu == true }"
          @click="(saveMenuKeyposition(index, item), clickedMenuKey = index, clickedMenukeys())">
          <span class="flex flex-col items-center justify-center w-full h-full rounded-lg border border-gray-500 "><span
              class="flex justify-center items-center">{{ item ? item.lngAmtCode : '' }}</span><span
              class="flex justify-center items-center">{{ item ? item.strKeyName : '' }}</span></span>
        </div>
      </VueDraggableNext>
    ...
    <div v-if="changeGrid == true" class="flex flex-col ml-3 w-10 h-9/12 mt-[70px] space-y-7">
      <div class="relative w-[50%]  right-9"><button class="whitebutton mr-20"
          @click="changeGrid = false; changeRowData();">돌아가기</button></div>
      <div class="flex flex-col ml-2 w-full h-[75%] ">
        ...
        <button class="border border-[#CCCCCC] w-full h-full rounded-md" @click="showNext"><img
            src="../../assets/arrow_down_wo_border.svg" alt=""></button>
      </div>&nbsp;&nbsp;&nbsp;{{ clickedGroupPage }} / 5
    </div>
