<!-- /*--############################################################################
  # Filename : MST37_072INS.vue                                               
  # Description : 마스터관리 > 메뉴 마스터 > 옵션 관리   
  # Date :2026-04-10                                                        
  # Author : 권지안                     
################################################################################*/ -->
<template>
  ...
  <div class="bg-gray-200 rounded-lg px-2 py-2">
    ...
    <div
      v-if="isStoreScopeFeatureEnabled"
        ...
        <div class="h-4/6 w-2/3" v-show="currentMenu == false">
          ...
          <div class="ml-10 mt-5 w-full h-[140%]">
            ...
            <Realgrid
              ...
              @updatedRowData2="updatedRowData3"
        ...
        <div class="h-4/6 w-2/3" v-show="currentMenu == true">
          ...
          <div class="ml-10 mt-5 w-full h-[140%]">
            ...
            <Realgrid
              ...
              @updatedRowData2="updatedRowData6"
...
import {
  ...
  getAllOptionManageData2_01101,
  getMostColumnMenuList,
  getOptionCompositionList,
  saveOptions,
  saveOptions2,
  saveNewOption1,
  saveNewOption2,
...
const hasChainOrderChanged = (
  ...
  const normalizeRows = (rows = []) =>
    rows
      ...
      .map((item) => ({
        lngCode: Number(item.lngCode) || 0,
        chain: Array.from({ length: chainCount }, (_, idx) =>
          Number(item[`${chainPrefix}${idx + 1}`]) || 0
        ),
      }))
      .sort((a, b) => a.lngCode - b.lngCode);
  ...
  return current.some((item, rowIdx) => {
    const target = original[rowIdx];
    if (!target || item.lngCode !== target.lngCode) {
      return true;
    }
    return item.chain.some((value, chainIdx) => value !== target.chain[chainIdx]);
  });
};
...
const getEffectiveStoreCd = () => {
  ...
  const selected = normalizeScopeStoreCd(scopeStoreCd.value || pickedStoreCd.value);
  return selected || nowStoreCd.value;
};
...
const clickedRowData2 = (newValue) => {
  ...
  for (var i = 0; i < 20; i++) {
    if (newValue[2 + i] != "0") {
      ...
      //comsole.log(filteredrowData4.value);
    }
  }
...
const searchButton = async () => {
  ...
  const commonMenuFlag =
    store.state.userData.intCommonMenu ?? store.state.userData.lngCommonMenu;
  if (nowStoreCd.value == "0" && String(commonMenuFlag) === "0") {
    Swal.fire({
      title: "경고",
      text: "매장을 선택하세요.",
      icon: "warning",
  ...
  try {
    ...
    const menuListStoreCd = isStoreScopeFeatureEnabled.value
      ? 0
      : nowStoreCd.value;

    // console.log(menuListStoreCd);

    let res3;
    let res4;
    let res5;
    if (isStoreScopeFeatureEnabled.value && dataScope.value === "STORE") {
      res3 = await getAllOptionManageData2_01101(groupCd.value, effectiveStoreCd);
    } else {
      res3 = await getAllOptionManageData(groupCd.value, effectiveStoreCd);
    }
    
    res5 = await getOptionCompositionList(groupCd.value, menuListStoreCd);
    res4 = await getMostColumnMenuList(groupCd.value,menuListStoreCd);
    
    // console.log(res5);

    rowData1.value = res3.data.OPTIONMANAGE;
    rowData2.value = res5.data.MENULIST;
...
const saveButton = async () => {
  ...
  const hasOptionCompositionChanged = hasChainOrderChanged(
    updatedRowData4.value,
    confirmitem.value,
    "lngChainMenu",
    21
  );
  const hasOptionGroupCompositionChanged = hasChainOrderChanged(
    updatedRowData5.value,
    confirmitem2.value,
    "lngChainGroup",
    20
  );
  if (currentMenu.value == false) {
    ...
    if (
      ...
      !hasOptionGroupCompositionChanged
    ) {
      Swal.fire({
        title: "경고",
        text: "변경된 사항이 없습니다.",
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        if (currentMenu.value == false) {
          ...
          const saveOptionsApi =
            isStoreScopeFeatureEnabled.value && dataScope.value === "STORE"
              ? saveNewOption1
              : saveOptions;
          const res = await saveOptionsApi(
            groupCd.value,
            effectiveStoreCd,
            lngCodes.join(","),
            strNames.join(","),
          ...
          const saveOptions2Api =
            isStoreScopeFeatureEnabled.value && dataScope.value === "STORE"
              ? saveNewOption2
              : saveOptions2;
          const res = await saveOptions2Api(
            groupCd.value,
            effectiveStoreCd,
            lngCodes2.join(","),
            strNames2.join(","),
