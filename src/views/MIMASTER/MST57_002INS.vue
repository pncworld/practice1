<template>
  <div class="flex justify-start pl-4 pt-4">
    ...
    <div class="absolute right-48 space-x-3 flex"><button class="button search"
        @click="searchPosMenu">조회</button><button class="button save" @click="savePosMenu">저장</button></div>
  </div>
  <br>
  ...
  <div class="border border-black w-96 h-96 ml-3 mt-10 overflow-auto flex flex-col">
    <div v-for="(mainCategory, mainIndex) in majorGroup" :key="mainIndex" class="mb-4 flex flex-col items-start">
      <button @click="toggleMain(mainIndex)"
      ...
      <div v-show="activeMain[mainIndex]" class="ml-4 mt-2 flex flex-col items-start space-y-2">
        <div v-for="(subCategory, subIndex) in mainCategory.subGroup" :key="subIndex">
          <button @click="toggleSub(subIndex)"
          ...
          <div v-show="activeSub[subIndex]" v-for="(item, itemIndex) in subCategory.menuList" :key="itemIndex"
            class="flex w-72 justify-between ml-16">
            <div class=""><button
                @click="addMenu(item.menuCd, item.menuNm, mainCategory.majorGroupNm, subCategory.subGroupNm, item.fileName)">{{
                item.menuNm }}/{{ item.lngPrice }}원 </button></div><button
              @click="addMenu(item.menuCd, item.menuNm, mainCategory.majorGroupNm, subCategory.subGroupNm)"
              class="whitebutton">추가</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <Container :onDrop="onDrop">
      ...
      <Draggable v-for="item in items">
        <div class="grid grid-cols-8 h-auto">
          <div class="h-16 flex items-center justify-center"><font-awesome-icon icon="bars" /></div>
          <div class="h-16 flex items-center justify-center">{{ item?.intKeySeq }}</div>
          <div class="h-16 flex items-center justify-center"><img v-if="item.fileName"
              :src="`http://www.pncoffice.net/MenuImage/Image/${item.fileName}`" alt="" class="w-full h-full"><img
              v-if="!item.fileName" src="../../assets/noImage.png" alt="">
          </div>
          <div class="h-16 flex items-center justify-center">{{ item?.lngKeyscrNo }}</div>
          <div class="h-16 flex items-center justify-center">{{ item?.strKeyName }}</div>
          <div class="h-16 flex items-center justify-center">{{ item?.majorName }}</div>
          <div class="h-16 flex items-center justify-center">{{ item?.subName }}</div>
          <div class="h-16 flex items-center justify-center"><button
              @click="deleteitem(item?.lngKeyscrNo, item?.intKeySeq)"><img src="../../assets/trash.svg"
                alt=""></button></div>
        </div>
      </Draggable>
    </Container>
