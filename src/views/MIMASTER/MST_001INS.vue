<template>
  ...
  <div class="flex justify-start  space-x-5 bg-gray-200 rounded-lg md:h-16 h-24 items-center">
    ...
    <div class="">
      ...
      <select
        ...
        <option :value="i.lngKeyscrNo" v-for="i in getScreenNos">[{{ i.lngKeyscrNo }}]</option>
      </select>
    </div>
  </div>
  <div class="z-[90]">
    <DupliPopUp6 :isVisible="showPopup2" @close="showPopup2 = false" :storeCd="nowStoreCd" :storeNm="clickedStoreNm"
      :areaCd="nowStoreAreaCd" :posNo="posNo" :progname="'MST01_004INS_VUE'" :dupliapiname="'DUPLITABLEKEY'" :progid="2"
      :poskiosk="'getStoreAndPosList3'" naming2="테이블">
    </DupliPopUp6>
  ...
  <div v-if="showSetScreenKey" class="fixed inset-0 bg-gray-500 bg-opacity-50 flex justify-center items-center z-[89]">
    <div class="bg-white p-6 rounded shadow-lg w-[25%] h-[40%] ">
      ...
      <div class="flex flex-col justify-start h-12">
        ...
        <div class="h-full w-full rounded-lg"><input type="text"
            class="border border-gray-400 pl-1 h-full w-full rounded-lg" v-model="currScreenNm"></div>
      </div>
  ...
  <div class="flex ml-32 -mt-0 w-[80%] h-16 flex-row justify-end items-center gap-2 z-[88]">

    <button class="w-10 flex-shrink-0 flex items-center" @click="initAllTable">
      <img src="../../assets/Btn_46_refresh.svg" alt="">
    </button>
    <button class="button primary w-[150px] h-[40px] flex justify-center" @click="addScreenKey2()"> 화면키 추가</button>
    <!-- <button class="button primary w-[150px] h-[40px] flex justify-center" @click="addScreenKey()">신규 화면키 추가</button> -->
  ...
  <div class="flex z-81 mt-2">
    ...
    <div
      ...
      <div class="flex justify-center items-center gap-3">
        <button :disabled="clickTable == false" @click="shapeclick(0)"
          :class="clickedShape == 0 ? 'bg-blue-200' : 'bg-white'"
          class="h-full w-[25%] flex justify-center items-center"><img src="../../assets/palette1.svg" alt=""></button>
        <button :disabled="clickTable == false" @click="shapeclick(1)"
          :class="clickedShape == 1 ? 'bg-blue-200' : 'bg-white'"
          class="h-full w-[25%] flex justify-center items-center"><img src="../../assets/palette2.svg" alt=""></button>
        <button :disabled="clickTable == false" @click="shapeclick(2)"
          :class="clickedShape == 2 ? 'bg-blue-200' : 'bg-white'"
          class="h-full w-[25%] flex justify-center items-center"><img src="../../assets/palette3.svg" alt=""></button>
        <button :disabled="clickTable == false" @click="shapeclick(3)"
          class="h-full w-[25%] flex justify-center items-center"
          :class="clickedShape == 3 ? 'bg-blue-200' : 'bg-white'"><img src="../../assets/palette4.svg" alt=""></button>
      </div>
      <div class="grid grid-cols-2">
        <div class="bg-gray-100 font-semibold flex justify-center items-center">테이블 코드</div>
        <div class="flex justify-center items-center "><input v-if="!clickedtableCode.toString().includes('new')"
            type="text" class="border rounded-lg border-gray-200 px-2" v-model="clickedtableCode" disabled><input
            v-if="clickedtableCode.toString().includes('new')" type="text"
            class="border rounded-lg border-gray-200 px-2" value="" disabled></div>
      </div>
      <div class="grid grid-cols-2">
        <div class="bg-gray-100 font-semibold flex justify-center items-center">테이블 명</div>
        <div class="flex justify-center items-center"><input type="text" class="border rounded-lg border-gray-200 px-2"
            v-model="clickedtableNm" @input="changetableProperty" :disabled="clickedtableNo == 1"></div>
      </div>
      <div class="grid grid-cols-2">
        <div class="bg-gray-100 font-semibold flex justify-center items-center">좌석 수</div>
        <div class="flex justify-center items-center"><input type="text" class="border rounded-lg border-gray-200 px-2"
            v-model="clickedtableSeats" @input="changetableProperty" :disabled="clickedtableNo == 1"></div>
      </div>
      <div class="grid grid-cols-4">
        <div class="bg-gray-100 font-semibold flex justify-center items-center">가로 위치(x)</div>
        <div class="flex justify-center items-center"><input type="text"
            class="border rounded-lg border-gray-200 px-2 w-full" v-model="clickedtableX" disabled></div>
        <div class="bg-gray-100 font-semibold flex justify-center items-center">세로 위치(Y)</div>
        <div class="flex justify-center items-center"><input type="text"
            class="border rounded-lg border-gray-200 px-2 w-full" v-model="clickedtableY" disabled></div>
      </div>
      <div class="grid grid-cols-4">
        <div class="bg-gray-100 font-semibold flex justify-center items-center">너비</div>
        <div class="flex justify-center items-center"><input type="text"
            class="border rounded-lg border-gray-200 px-2 w-full" v-model="clickedtableW" disabled></div>
        <div class="bg-gray-100 font-semibold flex justify-center items-center">높이</div>
        <div class="flex justify-center items-center"><input type="text"
            class="border rounded-lg border-gray-200 px-2 w-full" v-model="clickedtableH" disabled></div>
      </div>
      ...
      <div>
        <div class="grid grid-rows-2 grid-cols-7 gap-1 mt-5 ml-5">
          <button class="bg-white w-10 h-10 border border-gray-300 rounded-lg "
            :class="clickedtableColor == '#FFFFFF' ? '!border-black' : ''" @click="setColor(1)"></button>
          <button class="bg-[#BACCFF] w-10 h-10 border border-gray-300 rounded-lg "
            :class="clickedtableColor == '#BACCFF' ? '!border-black' : ''" @click="setColor(2)"></button>
          <button class="bg-[#B3EAFF] w-10 h-10 border border-gray-300 rounded-lg "
            :class="clickedtableColor == '#B3EAFF' ? '!border-black' : ''" @click="setColor(3)"></button>
          <button class="bg-[#CFFFAB] w-10 h-10 border border-gray-300 rounded-lg "
            :class="clickedtableColor == '#CFFFAB' ? '!border-black' : ''" @click="setColor(4)"></button>
          <button class="bg-[#FFDDBA] w-10 h-10 border border-gray-300 rounded-lg "
            :class="clickedtableColor == '#FFDDBA' ? '!border-black' : ''" @click="setColor(5)"></button>
          <button class="bg-[#FFC5C5] w-10 h-10 border border-gray-300 rounded-lg "
            :class="clickedtableColor == '#FFC5C5' ? '!border-black' : ''" @click="setColor(6)"></button>
          <button class="bg-[#D5C5FF] w-10 h-10 border border-gray-300 rounded-lg "
            :class="clickedtableColor == '#D5C5FF' ? '!border-black' : ''" @click="setColor(7)"></button>
          <button class="bg-[#C3C3C3] w-10 h-10 border border-gray-300 rounded-lg "
            :class="clickedtableColor == '#C3C3C3' ? '!border-black' : ''" @click="setColor(8)"></button>
          <button class="bg-[#7699FF] w-10 h-10 border border-gray-300 rounded-lg "
            :class="clickedtableColor == '#7699FF' ? '!border-black' : ''" @click="setColor(9)"></button>
          <button class="bg-[#5DD2FF] w-10 h-10 border border-gray-300 rounded-lg "
            :class="clickedtableColor == '#5DD2FF' ? '!border-black' : ''" @click="setColor(10)"></button>
          <button class="bg-[#9CFA55] w-10 h-10 border border-gray-300 rounded-lg "
            :class="clickedtableColor == '#9CFA55' ? '!border-black' : ''" @click="setColor(11)"></button>
          <button class="bg-[#FFB162] w-10 h-10 border border-gray-300 rounded-lg "
            :class="clickedtableColor == '#FFB162' ? '!border-black' : ''" @click="setColor(12)"></button>
          <button class="bg-[#FF9191] w-10 h-10 border border-gray-300 rounded-lg "
            :class="clickedtableColor == '#FF9191' ? '!border-black' : ''" @click="setColor(13)"></button>
          <button class="bg-[#AB8CFF] w-10 h-10 border border-gray-300 rounded-lg "
            :class="clickedtableColor == '#AB8CFF' ? '!border-black' : ''" @click="setColor(14)"></button>
        </div>
      </div>
      <div class="grid grid-cols-2 border border-y-black">
        <div @click="duplicateTable" class="border border-x-black flex justify-center items-center"
          :disabled="clickTable == false"><button class="w-full h-full">복사</button></div>
        <div @click="deleteTable" class="flex justify-center items-center border border-x-black "
          :disabled="clickTable == false"><button class="w-full h-full">삭제</button></div>
      </div>
    </div>
  </div>
...
const existedScreenKeys = ref([{ strKeyName: 1, lngKeyscrNo: 1 }, { strKeyName: 2, lngKeyscrNo: 2 }, { strKeyName: 3, lngKeyscrNo: 3 }, { strKeyName: 4, lngKeyscrNo: 4 }, { strKeyName: 5, lngKeyscrNo: 5 }, { strKeyName: 6, lngKeyscrNo: 6 }, { strKeyName: 7, lngKeyscrNo: 7 }, { strKeyName: 8, lngKeyscrNo: 8 }, { strKeyName: 9, lngKeyscrNo: 9 }, { strKeyName: 10, lngKeyscrNo: 10 }, { strKeyName: 11, lngKeyscrNo: 11 }, { strKeyName: 12, lngKeyscrNo: 12 }, { strKeyName: 13, lngKeyscrNo: 13 }, { strKeyName: 14, lngKeyscrNo: 14 }, { strKeyName: 15, lngKeyscrNo: 15 }])
...
const getScreenNos = ref([{ lngKeyscrNo: 1 }, { lngKeyscrNo: 2 }, { lngKeyscrNo: 3 }, { lngKeyscrNo: 4 }, { lngKeyscrNo: 5 }, { lngKeyscrNo: 6 }, { lngKeyscrNo: 7 }, { lngKeyscrNo: 8 }, { lngKeyscrNo: 9 }, { lngKeyscrNo: 10 }, { lngKeyscrNo: 11 }, { lngKeyscrNo: 12 }, { lngKeyscrNo: 13 }, { lngKeyscrNo: 14 }, { lngKeyscrNo: 15 }])
...
watch(selectedScreenNo, () => {
  if (selectedScreenNo.value != 0) {
    searchButton()
  }
})
...
const searchButton = async () => {
  ...
  try {
    ...
    let res2 = await getTableList2(groupCd.value, nowStoreCd.value, nowStoreAreaCd.value, posNo.value, selectedScreenNo.value)
    tableList.value = res2.data.TABLELISTS
    filteredtableList.value = tableList.value.map(item => ({
      ...item,
    ...
    existedScreenKeys.value = [{ strKeyName: 1, lngKeyscrNo: 1 }, { strKeyName: 2, lngKeyscrNo: 2 }, { strKeyName: 3, lngKeyscrNo: 3 }, { strKeyName: 4, lngKeyscrNo: 4 }, { strKeyName: 5, lngKeyscrNo: 5 }, { strKeyName: 6, lngKeyscrNo: 6 }, { strKeyName: 7, lngKeyscrNo: 7 }, { strKeyName: 8, lngKeyscrNo: 8 }, { strKeyName: 9, lngKeyscrNo: 9 }, { strKeyName: 10, lngKeyscrNo: 10 }, { strKeyName: 11, lngKeyscrNo: 11 }, { strKeyName: 12, lngKeyscrNo: 12 }, { strKeyName: 13, lngKeyscrNo: 13 }, { strKeyName: 14, lngKeyscrNo: 14 }, { strKeyName: 15, lngKeyscrNo: 15 }]
    const currScreenKeys = tableList.value.filter(item => item.intKeyNo == 1).map(item => item.lngKeyscrNo)
    existedScreenKeys.value = existedScreenKeys.value.filter(item => item.lngKeyscrNo != selectedScreenNo.value).filter(item => !currScreenKeys.includes(item.lngKeyscrNo))
...
function addNewWidget() {
  ...
  const node = {
    w: 5, // 너비
    h: 5,
    intScreenNo: selectedScreenNo.value,
    lngCount: 0,
    intKeyNo: 13,
  ...
  const tablelength = tableList.value.filter(item => item.intKeyNo == 13).length;
  const addedWidget = gridItems.find(item => item.gridstackNode.id === node.id);
  if (validate2 == true || tablelength >= 35) {
  ...
  filteredtableList.value.push({
    intScreenNo: Number(selectedScreenNo.value),
    strName: '신규' + sequence.value,
    lngShape: 0,
    intKeyNo: 13,
    lngKeyColor: 16777215,
...
function addScreenKey() {
  const currcount = tableList.value.length
  const minValue = Math.min(...tableList.value.filter(item => item.intKeySeq !== undefined).map(item => item.intKeySeq));
  console.log(currcount)
  console.log(minValue)
  if (maxtableseq.value != -1 && maxtableseq.value <= minValue + currcount) {
    Swal.fire({
      title: '테이블추가오류',
      text: '더이상 테이블을 추가할 수 없습니다.',
      icon: 'info',
      confirmButtonText: '확인'
    })
    return;
  ...
  const node = {
    ...
    lngCount: 0,
    lngKeyColor: '16777215',
    lngKeyscrNo: 'screen' + (sequence.value),
    lngShape: 0,
    strName: '새화면키' + sequence.value
  };
  // id는 count 값을 사용
  node.id = String('new' + (++sequence.value));
  ...
  if (textElement) {
    const newDiv = document.createElement('div');
    newDiv.innerText = '새화면키' + (sequence.value) // 텍스트 설정
    newDiv.style.position = 'absolute';
    newDiv.style.left = '0'
    newDiv.style.zIndex = '81'
    textElement.insertAdjacentElement('afterend', newDiv);
  ...
  widgetElement.addEventListener('click', function () {
    ...
    clickedShape.value = (Number(finditem.lngShape))
    clickedtableCode.value = finditem.id
    clickedtableNo.value = finditem.intKeyNo
    clickedtableNm.value = finditem.strName
    clickedtableX.value = finditem.x * 125
    clickedtableY.value = finditem.y * 125
    clickedtableW.value = finditem.w * 120
...
function addScreenKey2() {
  ...
  const existed = existedScreenKeys.value.shift();
  if (existed == undefined) {
    Swal.fire({
      title: '화면키추가오류',
      text: '더이상 화면키를 추가할 수 없습니다.',
      icon: 'info',
      confirmButtonText: '확인'
    })
    return;
  }
  const existingNm = existed.strKeyName
  const existingNo = existed.lngKeyscrNo
  // filteredtableList에서 아이템을 가져옴, 없으면 기본 값 설정
  const node = {
    w: 5, // 너비
    h: 5,
    ...
    intKeyNo: 1,
    lngCount: 0,
    lngKeyColor: '16757090',
    lngKeyscrNo: existingNo,
    lngShape: 0,
    strName: existingNm
  };
  // id는 count 값을 사용
  ...
  if (widgetElement) {
    widgetElement.style.backgroundColor = ''
    const resizeHandle = widgetElement.querySelector('.grid-stack-item-content')
    resizeHandle.style.backgroundColor = decimalToRGB('16757090')
    resizeHandle.classList.add('rectangle')
  ...
  if (textElement) {
    const newDiv = document.createElement('div');
    newDiv.innerText = existingNm  // 텍스트 설정
    newDiv.style.position = 'absolute';
    newDiv.style.left = '0'
    newDiv.style.zIndex = '81'
    textElement.insertAdjacentElement('afterend', newDiv);
...
const deleteTable = () => {
  ...
  console.log(filteredtableList.value)
  const finditem2 = tableList.value.find(item => item.lngKeyscrNo == clickedtableCode.value);
  if (finditem2) {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        res2 = await saveTables2(groupCd.value, nowStoreCd.value, posNo.value, nowStoreAreaCd.value, selectedScreenNo.value, ids.join(',')
          , lngKeyColors.join(','), lngShapes.join(','), strNames.join(','), lngCounts.join(',')
          , xs.join(','), ys.join(','), ws.join(','), hs.join(','), newtableNm.join(','), newtableCount.join(','), intKeyNo.join(','))
...
const changetableProperty = (e) => {
  ...
  const tableItem = tableList.value.find(item2 => item2.lngKeyscrNo == clickedtableCode.value)
  if (tableItem) {
...
const initAllTable = () => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      tableList.value = tableList.value.filter(item => item.intScreenNo !== selectedScreenNo.value)
      console.log(tableList.value)
    }
  })
