<template>
  ...
  <div
    class="flex ml-32 -mt-0 w-[80%] h-16 flex-row justify-end items-center gap-2 z-[88]">
    ...
    <button
      ...
      class="button primary w-[150px] h-[40px] flex justify-center"
      @click="addScreenKey2()">
      화면키 추가
    </button>
...
const existedScreenKeys = ref([
  { strKeyName: 1, lngKeyscrNo: 1 },
  { strKeyName: 2, lngKeyscrNo: 2 },
  { strKeyName: 3, lngKeyscrNo: 3 },
  { strKeyName: 4, lngKeyscrNo: 4 },
  { strKeyName: 5, lngKeyscrNo: 5 },
  { strKeyName: 6, lngKeyscrNo: 6 },
  { strKeyName: 7, lngKeyscrNo: 7 },
  { strKeyName: 8, lngKeyscrNo: 8 },
  { strKeyName: 9, lngKeyscrNo: 9 },
  { strKeyName: 10, lngKeyscrNo: 10 },
  { strKeyName: 11, lngKeyscrNo: 11 },
  { strKeyName: 12, lngKeyscrNo: 12 },
  { strKeyName: 13, lngKeyscrNo: 13 },
  { strKeyName: 14, lngKeyscrNo: 14 },
  { strKeyName: 15, lngKeyscrNo: 15 },
]);
...
const searchButton = async () => {
  ...
  try {
    let res2 = await getTableList2(
      groupCd.value,
      nowStoreCd.value,
      nowStoreAreaCd.value,
      posNo.value,
      selectedScreenNo.value
    );
    ...
    const currScreenKeys = tableList.value
      .filter((item) => item.intKeyNo == 1)
      .map((item) => item.lngKeyscrNo);
    existedScreenKeys.value = existedScreenKeys.value
      .filter((item) => item.lngKeyscrNo != selectedScreenNo.value)
      .filter((item) => !currScreenKeys.includes(item.lngKeyscrNo));
  }
};
...
const changetableProperty = (e) => {
  ...
  const tableItem = tableList.value.find(
    (item2) => item2.lngKeyscrNo == clickedtableCode.value
  );
  if (tableItem) {
