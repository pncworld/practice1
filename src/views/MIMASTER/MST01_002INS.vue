<template>
  ...
  <div class="h-[54%] w-full">
    <div class="flex justify-end gap-1 mt-1"><button @click="addButton" class="whitebutton">신규</button> <button
        @click="deleteButton" class="whitebutton">삭제</button> </div>
    <Realgrid class="w-full h-full mt-2" :progname="'MST01_002INS_VUE'" :progid="1" :rowData="rowData"
      @clickedRowData="clickedRowData3" @selcetedrowData="selcetedrowData" :selectionStyle="'singleRow'"
      :initFocus="initFocus"
      :labelingColumns="'lngSupervisor,lngSaleType,lngMultiPriceGroupCode,lngJoinType,lngSubLease,lngStoreAttr,lngStoreArea'"
      @updatedRowData="updatedRowData" :valuesData="valuesData" :labelsData="labelsData" :deleteRow="deleted"
      :changeColid="changeColid" :changeRow="changeRow" :changeValue2="changeValue" :useCheckboxfordelete="true"
      :addRow4="addRow4" :addrowDefault="addrowDefault" :addrowProp="addrowProp" :addField="'new'"
      :rowStateeditable="false" :exporttoExcel="exExcel" :ExcelNm="exExcelNm" :changeNow="changeNow"></Realgrid>
  </div>
  <div class="relative left-0 top-4 mt-5">
    <div class="absolute grid grid-cols-6 grid-rows-10 gap-0 w-full">
      ...
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><input type="text" id="storeCode"
          class="text-sm border rounded-md w-full pl-2 h-7 disabled:bg-gray-300" v-model="lngStoreCode"
          :disabled="disableStoreCode" name="lngStoreCode" @input="updateGridValue" /></div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center bg-gray-100 text-blue-500">
        *매장명
      </div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><input type="text" id="storeCode"
          class="text-sm border rounded-md w-full pl-2 h-7 " v-model="strName" name="strName"
          @input="updateGridValue" /></div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center bg-gray-100">
        사업자번호
      </div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center rounded-se-xl"><input type="text"
          id="storeCode" class="text-sm border rounded-md w-full pl-2 h-7 " v-model="strRegistNo" name="strRegistNo"
          @input="updateGridValue" /></div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center bg-gray-100  text-blue-500">
        *가맹유형
      </div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><select
          class=" border rounded-md w-full h-7 pl-1" v-model="lngJoinType" name="lngJoinType" @change="updateGridValue">
      ...
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><select
          class="text-sm border rounded-md w-full h-7 pl-1 " v-model="lngMultiPriceGroupCode"
          name="lngMultiPriceGroupCode" @change="updateGridValue">
          <option value="-1" class="text-sm">선택</option>
          <option v-for="item in lngMultiPriceGroupCodes" :key="item.lngMultiPriceGroupCode"
            :value="item.lngMultiPriceGroupCode" class="text-sm">
      ...
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><input type="hidden" id="storeCode"
          class="text-sm border rounded-md w-full  h-7 pl-1" /></div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center bg-gray-100 text-blue-500">
        *매장구분
      </div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><select id="storeCode"
          class="border rounded-md w-full  h-7 pl-1" v-model="lngStoreAttr" name="lngStoreAttr"
      ...
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><select id="storeCode"
          class="text-sm border rounded-md w-full h-7 pl-1" v-model="lngSubLease" name="lngSubLease"
          @change="updateGridValue">
          <option value="-1" class="text-sm">선택</option>
      ...
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><select type="text" id="storeCode"
          class="text-sm border rounded-md w-full  h-7 pl-1" v-model="lngStoreArea" name="lngStoreArea"
          @change="updateGridValue">
          <option value="-1" class="text-sm">선택</option>
      ...
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><input type="text" id="storeCode"
          class="text-sm border rounded-md w-full pl-2 h-7 " v-model="strDirector" name="strDirector"
          @input="updateGridValue" /></div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center bg-gray-100">
        업종
      </div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><input type="text" id="storeCode"
          class="text-sm border rounded-md w-full pl-2 h-7 " v-model="strDealType" name="strDealType"
          @input="updateGridValue" /></div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center bg-gray-100">
        업태
      </div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><input type="text" id="storeCode"
          class="border rounded-md w-full  h-7 pl-2" v-model="strDealKind" name="strDealKind"
          @input="updateGridValue" /></div>
      ...
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><input type="date"
          v-model="dtmOpenDate" max="9999-12-31" class="border p-2 rounded h-full w-full" /></div>

      <div class="border flex h-7 items-center text-sm font-semibold justify-center bg-gray-100">
        폐점일자
      </div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><input type="date"
          v-model="dtmStop" max="9999-12-31" class="border p-2 rounded h-full w-full" />
      </div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center bg-gray-100">
        변환코드
      </div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><input type="text" id="storeCode"
          class="border text-sm rounded-md w-full p-2 h-7 pl-2" v-model="strConvCode" name="strConvCode"
          @input="updateGridValue" /></div>
      ...
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><input type="text" id="storeCode"
          class="border text-sm rounded-md w-full p-2 h-7 pl-2" v-model="strZipCode" name="strZipCode"
          @input="updateGridValue" /></div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center bg-gray-100">
        주소
      </div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center w-[200%]"><input type="text"
          id="storeCode" class="border text-sm rounded-md w-full h-7 p-2 pl-2" v-model="strAddress" name="strAddress"
          @input="updateGridValue" /></div>
      <div class="h-0 w-0">
      </div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><input type="text" id="storeCode"
          class="border text-sm rounded-md w-full pl-2 h-7 " v-model="strAddressEtc" name="strAddressEtc"
          @input="updateGridValue" /></div>
      ...
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><input type="text" id="storeCode"
          class="border text-sm rounded-md w-full pl-2 h-7 " v-model="strTel" name="strTel" @input="updateGridValue" />
      </div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center bg-gray-100">
        휴대폰번호
      </div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><input type="text" id="storeCode"
          class="border text-sm rounded-md w-full pl-2 h-7 " v-model="strPhone" name="strPhone"
          @input="updateGridValue" /></div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center bg-gray-100">
        팩스번호
      </div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><input type="text" id="storeCode"
          class="border text-sm rounded-md w-full pl-2 h-7 " v-model="strFax" name="strFax" @input="updateGridValue" />
      </div>

      <div class="border flex h-7 items-center text-sm font-semibold justify-center bg-gray-100">
        평수
      </div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><input type="number" id="storeCode"
          class="border text-sm rounded-md w-full pl-2 h-7 " v-model="lngFloorSpace" name="lngFloorSpace"
          @input="updateGridValue" /></div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center bg-gray-100">
        임차조건
      </div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><input type="number" id="storeCode"
          class="border text-sm rounded-md w-full pl-2 h-7 " v-model="lngLease" name="lngLease"
          @input="updateGridValue" /></div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center bg-gray-100">
        BEP
      </div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><input type="number" id="storeCode"
          class="border text-sm rounded-md w-full pl-2 h-7 " v-model="lngBEP" name="lngBEP" @input="updateGridValue" />
      </div>
      ...
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><select id="storeCode"
          class="text-sm border rounded-md w-full pl-1 h-7 " v-model="lngSaleType" name="lngSaleType"
          @change="updateGridValue">
          <option value="-1" class="text-sm">선택</option>
      ...
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><select id="storeCode"
          class="text-sm border rounded-md w-full pl-1 h-7 " v-model="lngSupervisor" name="lngSupervisor"
          @change="updateGridValue">
          <option value="-1" class="text-sm">선택</option>
      ...
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><input type="text" id="storeCode"
          class="border text-sm rounded-md w-full pl-2 h-7 " v-model="strDev1" name="strDev1"
          @input="updateGridValue" /></div>
      ...
      <div class="border flex h-7 items-center text-sm font-semibold justify-center"><input type="number" id="storeCode"
          class="border text-sm rounded-md w-full pl-2 h-7 " v-model="lngTable" name="lngTable"
          @input="updateGridValue" /></div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center bg-gray-100">
        매장이력
      </div>
      <div class="border flex h-7 items-center text-sm font-semibold justify-center rounded-ee-xl w-[300%]"><input
          type="text" id="storeCode" class="border text-sm rounded-md w-full pl-2 h-7 " v-model="strStoreHistory"
          name="strStoreHistory" @input="updateGridValue" /></div>
