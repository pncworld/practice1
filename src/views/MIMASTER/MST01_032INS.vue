<template>
  ...
  <div class="flex items-center justify-between">
    <h1 class="font-bold text-sm md:text-2xl ml-2">마스터자동다운로드 등록</h1>
    <div class="flex space-x-2">
      ...
      <button @click="deleteButton" class="button delete md:w-auto w-14">삭제</button>
    </div>
  </div>
  <br/>
  <br/>
  ...
  <div class="flex flex-wrap gap-4 items-center">
    ...
    <!-- 적용일 -->
    <div class="flex items-center space-x-2 w-60">
      <span class="whitespace-nowrap">적용일:</span>
...
const saveButton = async() => {
    ...
    const response = await master_save(  groupCd.value,  storeCd.value,  lngAreaCd.value,  posNo.value,  selectedrows, selectedDate.value);
  ...
  const deleteButton = async() => {
    ...
    const response = await master_delete(groupCd.value, selectedstoreCd, selectedAreaCd, selectedPosNo, selectedrows, selectedDate.value);
