<template>
  <div>
    ...
    <div class="flex justify-between items-center w-full overflow-y-auto">
        ...
        <div class="flex justify-center mr-9 space-x-2 pr-5">
          ...
          <button @click="saveButton" class="button save w-auto save">저장</button>
          <button @click="deleteButton" class="button save w-auto delete">삭제</button>
    ...
    <div class="flex justify-between  bg-gray-200 rounded-lg h-16 items-center z-10">
  ...
  <div class="grid grid-cols-[1fr,2fr] grid-rows-1 justify-between h-[85%] space-x-10 w-[97%] mt-1">
     <Realgrid :progname="'MST01_032INS_VUE'" :progid="1" :rowData="rowData" @checkedRowData="checkedRowData"></Realgrid>
     <Realgrid :progname="'MST01_032INS_VUE'" :progid="2" :rowData="rowData2" @checkedRowData="checkedRowData2"></Realgrid>
    </div>
</template>
...
import { deleteMasterList, getAreaList, getMaster2List, getMasterList, getPosList, insertMasterList, master_delete, master_save } from '@/api/master';
...
const searchButton = async () => {
    ...
    try {
      ...
      const res2 = await getMaster2List(groupCd.value, storeCd.value, lngAreaCd.value ,posNo.value, selectedDate.value)
...
const saveButton = async () => {
  ...
  try {
    ...
    const res = await insertMasterList(groupCd.value, storeCd.value, lngAreaCd.value ,posNo.value, saveTables.value.join(','), selectedDate.value)
    console.log(res)
  } catch (error) {
...
const deleteButton = async () => {
  ...
  try {
    ...
    const res = await deleteMasterList(groupCd.value, deleteTablesStoreCds.value.join(','), deleteTablesAreaCds.value.join(',') ,deleteTablesPosNos.value.join(','), deleteTables.value.join(','), selectedDate.value)
    console.log(res)
  } catch (error) {
