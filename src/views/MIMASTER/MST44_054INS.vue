<template>
  <div class="flex justify-between items-center w-full overflow-y-auto">
    ...
    <div class="flex justify-center mr-10 space-x-2 pr-5"><button @click="searchAmount"
  ...
  <div class="z-50">
    <DupliPopUp3 :isVisible="showPopup2" @close="showPopup2 = false" :storeCd="nowStoreCd"
      :poskiosk="'getStoreDistinctList'" :storeNm="clickedStoreNm" :areaCd="nowStoreAreaCd" :posNo="posNo"
      :screenType="currentpaymentCd" :screenList="screenList" :progname="'MST44_054INS_VUE'"
      :dupliapiname="'DUPLIPAYKEY'" :progid="2" :naming="'POS번호'">
    </DupliPopUp3>
  </div>
  ...
  <span class="h-5 mt-3 flex justify-between items-center w-[900px] ml-[700px]  z-40">
    <h1 class="font-bold text-xl z-40">결제키 설정</h1><span class="flex space-x-3 ml-32 pl-56 items-center">순서변경 &nbsp;
    ...
    <div class="flex flex-col w-3/5 h-5/6">
      ...
      <div class="h-full" v-show="currentMenu == false">
        ...
        <div class="ml-10 mt-0 w-full h-full">
          ...
          <Realgrid class="w-full h-full" :progname="'MST44_054INS_VUE'" :progid="1" :rowData="rowData"
    ...
    <div class="grid grid-rows-[1fr,4fr] grid-cols-1 ml-10 w-full h-full z-20">
      <div class="flex h-full w-[86%] mt-10 items-start justify-center" v-show="afterSearch">
        <div class="grid grid-cols-4 grid-rows-1 gap-x-1 ml-10 mt-0 w-full h-full">
          <div v-for="(item, index) in ScreenKeys" class="custom-grid-style w-[174px] h-[80px]"
            :class="{ '!bg-orange-500 !border-blue-700': clickedScreenKeyIndex == index && clickedScreenOrMenu == false }"
            @click="clickedScreenKeyIndex = index; clickedScreenKeys()">
            <p v-if="item.strScreenName != ''" @click="showKeys(item.intScreenNo);"
              class=" flex items-center justify-center w-full h-full"><button class="">{{ item.strScreenName
                }}</button><button
                @click.stop="editScreenKey(item.intScreenNo, item.strScreenName, item.itemDiscYn)"><font-awesome-icon
        ...
        <div class="flex  mt-5 ml-2 w-36 h-1/5  "><button
            class="border border-[#CCCCCC] w-full h-full rounded-md flex justify-center items-center"
            @click="showPrev"><img src="../../assets/table-left.svg" alt=""
              class="w-full h-full object-cover rounded-md"></button><button
            class="border border-[#CCCCCC] h-full w-full rounded-md flex justify-center items-center"
            @click="showScreenKeysOrder"><img src="../../assets/table-change_2.svg" alt=""></button><button
            class="border border-[#CCCCCC] w-full h-full rounded-md flex justify-center items-center"
            @click="showNext"><img src="../../assets/table-right.svg" alt=""
              class="w-full h-full object-cover rounded-md"></button></div>
      ...
      <div v-show="afterSearch" class="flex mt-4 w-11/12 h-5/6">
        <div class="flex w-full h-full ">
          <VueDraggableNext v-model="items" :move="onMove" @end="onEnd" animation="200"
            ...
            <div v-for="(item, index) in items" class="custom-grid-style"
              ...
              <span class="flex flex-col items-center justify-center w-full h-full rounded-lg"
                :class="{ '!bg-gray-300': index == 29 || item.itemDiscYn == 1 }"><span
                  class="flex justify-center items-center">{{ item ? item.lngKeyScrNo : '' }}</span><span
                  class="flex justify-center items-center">{{ item ? item.strName : '' }}</span></span>
            </div>
          </VueDraggableNext>
...
const onEnd = (evt) => {
  ...
  KeyList.value = KeyList.value.filter(item => (item.intScreenNo != clickedintScreenNo.value))
  console.log(KeyList.value)
...
const searchAmountList3 = (e) => {
  if (currentMenuorTLU.value == false) {
    ...
    const filteredList = AmountList.value.filter(item =>
...
const addKey = () => {
  ...
  console.log(clickedintScreenNo.value)
  if (foraddIndex == -1) {
    KeyList.value.push({ intKeyNo: currentpaymentCd.value, intKeySeq: clickedRealIndex.value, intPosNo: posNo.value, intScreenNo: clickedintScreenNo.value, lngKeyScrNo: Number(clickedCode.value), strName: clickedstrName.value, itemDiscYn: 0 })
  } else {
    KeyList.value[foraddIndex] = { intKeyNo: currentpaymentCd.value, intKeySeq: clickedRealIndex.value, intPosNo: posNo.value, intScreenNo: clickedintScreenNo.value, lngKeyScrNo: Number(clickedCode.value), strName: clickedstrName.value, itemDiscYn: 0 }
  }
...
const addTLUKey = () => {
  console.log(KeyList.value)
  const foraddIndex = KeyList.value.findIndex(item => item.intKeySeq == clickedRealIndex.value)
  console.log(foraddIndex)
  if (foraddIndex == -1) {
    KeyList.value.push({ intKeyNo: 6, intKeySeq: clickedRealIndex.value, intPosNo: posNo.value, intScreenNo: clickedintScreenNo.value, lngKeyscrNo: Number(clickedTLUCD.value), strKeyName: clickedTLUNM.value })
  } else {
    KeyList.value[foraddIndex] = { intKeyNo: 6, intKeySeq: clickedRealIndex.value, intPosNo: posNo.value, intScreenNo: clickedintScreenNo.value, lngKeyscrNo: Number(clickedTLUCD.value), strKeyName: clickedTLUNM.value }
  }
  showKeys(clickedintScreenNo.value)
}
...
const deletekey = () => {
  if (clickedScreenOrMenu.value == false) {
    ...
    KeyList.value = KeyList.value.filter(item => (item.intScreenNo != clickedintScreenNo.value || item.intKeySeq != clickedRealIndex.value))
    console.log(KeyList.value)
    showKeys(clickedintScreenNo.value)
  }
}
