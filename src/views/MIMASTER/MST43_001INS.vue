  ...
  <div class="h-[80%]">
    ...
    <div class="grid grid-rows-1 grid-cols-2 h-full w-full justify-center mt-2">
      ...
      <div class="w-[110%] ml-10 h-full">
        ...
        <Realgrid :progname="'MST43_001INS_VUE'" :progid="1" :rowData="rowData" @clickedRowData="clickedRowData"
          @selcetedrowData="selcetedrowData" @updatedRowData="updatedRowData" :selectionStyle="'singleRow'"
          :setNumberformatColumn="'lngRate'" :labelingColumns="'lngStoreCode'" :valuesData="valuesData"
          :labelsData="labelsData" :changeNow="changeNow" :changeValue2="changeValue2" :changeColid="changeColid"
          :changeRow="changeRow" @selectedIndex="selectedIndex" :addRow4="addRow" :deleteRow2="deleteRow"
          :addrowDefault="addrowDefault" :addrowProp="addrowProp" :rowStateeditable="false" :addField="'new'">
        </Realgrid>
      </div>
      ...
      <div class="grid grid-cols-[1fr,6fr] grid-rows-6 w-[70%] ml-44 h-[30%] ">
        ...
        <div class="border flex h-full items-center text-sm font-semibold justify-center rounded-tr-lg"><input
            ...
            name="gridvalue1" @input="changeInfo" disabled></div>
        <div class="border flex h-full items-center text-sm font-semibold justify-center bg-gray-100 text-blue-500">
          *매입사코드</div>
        <div class="border flex h-full items-center text-sm font-semibold justify-center"><input type="text"
            ...
            name="gridvalue2" @input="changeInfo" :disabled="addNew"></div>
        <div class="border flex h-full items-center text-sm font-semibold justify-center bg-gray-100">매입사명</div>
        <div class="border flex h-full items-center text-sm font-semibold justify-left "><input type="text"
            class="border text-sm rounded-md w-full pl-2 h-full" v-model="gridvalue3" name="gridvalue3"
            @input="changeInfo"></div>
        <div class="border flex h-full items-center text-sm font-semibold justify-center bg-gray-100 ">영문명</div>
        <div class="border flex h-full items-center text-sm font-semibold justify-center rounded-br-lg"><input
            type="text" class="border text-sm rounded-md w-full pl-2 h-full" v-model="gridvalue4" name="gridvalue4"
            @input="changeInfo"></div>
        <div class="border flex h-full items-center text-sm font-semibold justify-center bg-gray-100 ">적용비율</div>
        <div class="border flex h-full items-center text-sm font-semibold justify-center rounded-br-lg"><input
            type="text" class="border text-sm rounded-md w-full pl-2 h-full" v-model="gridvalue5" name="gridvalue5"
            @input="changeInfo"></div>
        <div class="border flex h-full items-center text-sm font-semibold justify-center bg-gray-100 rounded-bl-lg">
          인터페이스코드</div>
        <div class="border flex h-full items-center text-sm font-semibold justify-center rounded-br-lg"><input
            type="text" class="border text-sm rounded-md w-full pl-2 h-full" v-model="gridvalue6" name="gridvalue6"
            @input="changeInfo"></div>
        <input type="hidden" class="border text-sm rounded-md w-full pl-2 h-full disabled:bg-gray-200"
          v-model="gridvalue7" name="gridvalue7" @input="changeInfo" disabled>
