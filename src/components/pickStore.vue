    ...
    <div class="inline-block" v-if="showAreaCd">
     <span class="font-bold text-sm ">지역코드 : &nbsp;</span> <select :disabled="isDisabled4"  class="w-32 border border-gray-800 rounded-md p-2 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" v-model="selectedStoreAreaCd" @change="emitStoreAreaCd($event.target.value)">
      ...
      <div class="" v-if="showKioskNo">
        <span class="font-bold text-sm "> KIOSK번호 : &nbsp;</span> <select :disabled="isDisabled4"  class="w-32 text-sm border border-gray-800 rounded-md p-2 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" v-model="selectedKioskNo" @change="ischanged();">
          <option value="0">선택</option>
          <option :value="{ lngCode: item.lngCode , lngAreaCode: item.lngAreaCode} " v-for="item in storeKioskNo" :key="item.lngAreaCode">{{ item.strName }}</option>
        </select>
      </div>
...
const emit = defineEmits(['update:storeGroup' , 'update:storeType' , 'update:storeCd' ,'update:ischanged','update:ischanged2', 'GroupNm','storeNm','update:storeAreaCd','posNo','screenNo','kioskNo','updatePaymentType']);
...
if ( selectedPosNo.value !='0' && props.showScreenNo == true){
    const res2 = await getScreenList2(storeGroup.value[0].lngStoreGroup, selectedStoreCode.value , selectedPosNo.value.lngAreaCode, selectedPosNo.value.lngCode)
...
watch(selectedGroupCd ,(newValue) => {
  const GroupNm= store.state.storeGroup.filter(item => item.lngStoreGroup == selectedGroupCd.value)[0].strName
