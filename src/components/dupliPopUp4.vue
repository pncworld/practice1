    ...
    <div v-if="isVisible" class="fixed flex top-0 left-0 w-full h-full bg-black bg-opacity-50 z-50 justify-center items-center">
      <div class=" bg-white w-[30%] h-[80%] shadow-lg rounded-lg flex flex-col">
        ...
        <main class="popup-body flex-grow">
          ...
          <div class="grid grid-rows-2 grid-cols-[1fr,4fr] border-[1px] border-gray-400 w-[480px] ml-12 h-32 rounded-lg ">
            ...
            <div class="border-[1px] border-gray-400 rounded-bl-md flex justify-center items-center">KIOSK번호</div>
          ...
          <div class="grid grid-cols-[1fr,4fr,1fr] border-[1px] border-gray-400 w-[480px] ml-12 h-10 mt-5 rounded-lg ">
...
import Swal from "sweetalert2";
  const {isVisible ,storeCd,storeNm,posNo,areaCd , progname , progid ,dupliapiname ,poskiosk,nowscreenNo , screenList ,screenType} = defineProps({
   ...
   console.log(progid) 
      try {
       ...
       if(searchWord.value ==''){
        ...
        rowData.value = res.data.store.filter(item => item.strName.includes(searchWord.value) ||  item.lngStoreCode.toString().includes(searchWord.value))
       }
  ...
  try {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        store.state.loading = true;
       const res3 = await api2[dupliapiname](groupCd.value , storeCd,areaCd ,posNo , selectedScreen.value, screenType ,  storeCd2.join(','))
       console.log(res3)
