<template>
  <div class="bg-white text-gray-600  w-full overflow-auto h-full m-0">
    <div class="">
      <h1 class="flex text-2xl font-bold h-12 items-center bg-gray-100 p-5">{{cMenu}}</h1>
      
        <div class="flex h-14"><input type="text" class="rounded-2xl bg-transparent border border-solid placeholder-opacity-30 y-3 mx-2 my-2 px-3" style="height:39px; width:190px; border-radius: 19px;" placeholder="검색어입력"><button class="w-2/12"><img src="../assets/finder_icon.svg" class=" inline-block w-6/12" alt=""></button></div>
        <label class="bg-gray-100">
        <div class="flex justify-start bg-gray-100 h-8 items-center "><input type="checkbox" @click="showAll" class="ml-3" />전체</div>
    </label>
      <ul>
        <li v-for="category in categories" :key="category.lngProgramSub" :id="category.lngProgramSub" class="bg-gray-100 ">
          <button 
          ...
          >
           <span> {{ category.strTitle }}</span><img src="../assets/arrow2.png" alt="" class="transition-transform duration-300" :class="{'rotate-0': !rotateStates[category.lngProgramSub], '-rotate-90': rotateStates[category.lngProgramSub]}"
           >  
          </button>
          <ul v-if="isOpen(category.lngProgramSub)" class="pl-4 mt-2">
            <li v-for="sub in category.subcategories" :key="sub.id" >
              <div class="flex"><button @click="favoriteadd(sub.lngProgramID)"><img src="../assets/table_star.svg" alt="" v-if="!currentFavorite[sub.lngProgramID]"><img src="../assets/table_star-checked.svg" alt="" v-if="currentFavorite[sub.lngProgramID]"></button><a href="#" class="flex items-center bg-gray-100 text-gray-900  rounded hover:bg-blue-100 h-10" @click="selectCategory(sub.strUrl , sub.lngProgramID , sub.strTitle)">
                {{ sub.strTitle }} 
              </a>
            </div>
            </li>
