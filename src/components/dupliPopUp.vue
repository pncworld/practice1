<template>
    <div v-if="isVisible" class="fixed flex top-0 left-0 w-full h-full bg-black bg-opacity-50 z-50 justify-center items-center">
      <div class=" bg-white w-[30%] h-[80%] shadow-lg rounded-lg flex flex-col">
        <header class="popup-header flex justify-between pl-5 pt-3">
            <h1 class="font-bold text-2xl">메뉴키 복사</h1>
            <button class="border border-gray-300 button primary rounded-md mr-5" @click="dupliStore">복사</button>
        </header>
        <main class="popup-body flex-grow">
          ...
          <div class="flex justify-between font-bold text-xl ml-12"><div>대상 매장 선택</div><div><button class="whitebutton mr-5" @click="showStoreList">조회</button></div></div>
        ...
        <div class="w-[480px] h-[380px] ml-12 flex justify-center"><realgrid :progname="'MST01_011INS_VUE'" :progid="1" :rowData="rowData" :showGrid="showGrid" :showCheckBar="true" :searchWord="searchWord" @selcetedrowData="selcetedrowData"></realgrid></div>
        </main>
        <footer class="flex justify-end p-5 mt-auto">
         <div class="border border-gray-300 rounded-md w-20 h-10"><button @click="close" class="w-full h-full">닫기</button></div>
        </footer>
      </div>
    </div>
  </template>
...
import { getStoreAndPosList } from "@/api/common";
import { useStore } from "vuex";
  const {isVisible ,storeCd,storeNm,posNo} = defineProps({
    isVisible: { type: Boolean, default: false }, // 팝업 가시성 관리
    storeCd : { type : String , default: ''} ,
    storeNm : { type : String , default: ''} ,
    posNo : { type : String , default: ''} ,
  });
